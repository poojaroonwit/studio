...
export async function parseCsvImportFile(file: File, defaultMatchCriteria: string): Promise<CsvImportParseResult> {
  if (file.size > MAX_FILE_SIZE) {
    return {
      ok: false,
      body: { message: `File too large. Maximum size is ${MAX_FILE_SIZE / (1024 * 1024)}MB` },
      status: 400,
    };
  }
