      ...
      <div className="mx-auto grid min-h-[720px] max-w-[1120px] overflow-hidden rounded-lg border border-border bg-card lg:grid-cols-[250px_minmax(0,1fr)]">
        <aside className="border-b border-border bg-muted/40 lg:border-b-0 lg:border-r">
          <div className="border-b border-border p-4">
            <h1 className="text-sm font-semibold text-foreground">Approval routes</h1>
          ...
          <nav aria-label="Compensation approval routes" className="divide-y divide-border">
            {visibleRoutes.map(route => (
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-xs font-semibold text-foreground">{route.name}</span>
                  <span className="mt-1.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
                    <span className={cn('h-2 w-2 rounded-full', route.isDefault ? 'bg-emerald-400' : route.isActive ? 'bg-blue-400' : 'bg-muted/400')} />
                    {route.isDefault ? 'Default route' : route.isActive ? 'Custom route' : 'Inactive'}
                  </span>
                  <span className="mt-1 block text-[11px] text-muted-foreground">{route.steps.length} approval step{route.steps.length === 1 ? '' : 's'}</span>
                </span>
        ...
        <section className="min-w-0">
          <header className="flex flex-col gap-4 border-b border-border px-5 py-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="truncate text-lg font-semibold tracking-[-0.01em] text-foreground">{selectedRoute.name}</h2>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Configure the ordered reviews applied when a compensation cycle is submitted.</p>
            </div>
          ...
          <section className="border-b border-border px-5 py-5">
            <div className="flex items-end justify-between gap-4">
              <div><h3 className="text-sm font-semibold text-foreground">Approval sequence</h3><p className="mt-1 text-xs text-muted-foreground">Approvals occur in the order shown.</p></div>
            </div>
            ...
            <div className="mt-5 flex snap-x gap-2 overflow-x-auto pb-2">
              {selectedRoute.steps.map((step, index) => (
                <div key={`${selectedRoute.id}-${index}`} className="flex shrink-0 snap-start items-center gap-2">
                  <article className="w-[184px] overflow-hidden rounded-md border border-border bg-background">
                    <div className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <Input aria-label={`Responsibility for step ${index + 1}`} value={step.title} disabled={!canEdit} onChange={event => updateStep(index, { title: event.target.value })} className="h-8 border-0 bg-transparent px-0 text-xs font-semibold text-foreground focus-visible:ring-0" />
                      </div>
                      <label className="mt-4 block text-[10px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Approver role</label>
                      <div className="mt-1 flex items-center gap-2 text-xs text-foreground/80"><UserRound className="h-4 w-4 text-blue-700 dark:text-blue-400" /><Input aria-label={`Approver role for step ${index + 1}`} value={step.role} disabled={!canEdit} onChange={event => updateStep(index, { role: event.target.value })} className="h-8 border-0 bg-transparent px-0 text-xs focus-visible:ring-0" /></div>
                    </div>
                    <div className="grid grid-cols-4 divide-x divide-border border-t border-border">
                      <span className="grid h-9 place-items-center text-muted-foreground"><GripVertical className="h-4 w-4" /></span>
                      <button type="button" disabled={!canEdit || index === 0} onClick={() => moveStep(index, -1)} className="grid h-9 place-items-center text-muted-foreground hover:text-foreground disabled:opacity-25 dark:hover:text-white" aria-label={`Move ${step.title} left`}><ArrowUp className="h-4 w-4 -rotate-90" /></button>
                      <button type="button" disabled={!canEdit || index === selectedRoute.steps.length - 1} onClick={() => moveStep(index, 1)} className="grid h-9 place-items-center text-muted-foreground hover:text-foreground disabled:opacity-25 dark:hover:text-white" aria-label={`Move ${step.title} right`}><ArrowDown className="h-4 w-4 -rotate-90" /></button>
                  ...
                  {index < selectedRoute.steps.length - 1 && <ArrowRight className="h-5 w-5 shrink-0 text-muted-foreground" />}
                </div>
              ))}
              <button type="button" disabled={!canEdit || selectedRoute.steps.length >= 8} onClick={() => updateSelected({ steps: [...selectedRoute.steps, { role: 'Approver', title: 'Review request' }] })} className="flex min-h-[184px] w-[145px] shrink-0 flex-col items-center justify-center rounded-md border border-dashed border-border text-xs text-muted-foreground transition hover:border-primary hover:text-primary disabled:opacity-40 dark:hover:text-blue-300"><Plus className="mb-3 h-6 w-6" />Add approval step</button>
            </div>
          </section>
          ...
          <div className="grid md:grid-cols-2">
            <section className="border-b border-border p-5 md:border-b-0 md:border-r">
              ...
              <div className="mt-4 space-y-4">
                <label className="block text-xs font-medium text-foreground/80">Path name<Input className="mt-1.5 h-9 border-border bg-background text-xs" value={selectedRoute.name} disabled={!canEdit} onChange={event => updateSelected({ name: event.target.value })} /></label>
                <label className="block text-xs font-medium text-foreground/80">Path ID<Input className="mt-1.5 h-9 border-border bg-background text-xs text-muted-foreground" value={selectedRoute.id} disabled /></label>
                <label className="block text-xs font-medium text-foreground/80">Description<Textarea className="mt-1.5 min-h-24 border-border bg-background text-xs" value={selectedRoute.description} disabled={!canEdit} onChange={event => updateSelected({ description: event.target.value })} /></label>
              </div>
            </section>

            <section className="border-b border-border p-5 md:border-b-0">
              <h3 className="text-sm font-semibold text-foreground">Submission behavior</h3>
              <dl className="mt-4 divide-y divide-border text-xs">
                <div className="pb-4"><dt className="font-medium text-foreground/80">Requester record</dt><dd className="mt-1 leading-5 text-muted-foreground">The requester is recorded automatically before these reviewers.</dd></div>
                <div className="py-4"><dt className="font-medium text-foreground/80">Step reassignment</dt><dd className="mt-1 leading-5 text-muted-foreground">Route owners can reassign a pending step before final approval.</dd></div>
                <div className="pt-4"><dt className="font-medium text-foreground/80">Completion policy</dt><dd className="mt-1 leading-5 text-muted-foreground">Every step in the sequence must be approved.</dd></div>
              </dl>
              {!selectedRoute.isDefault && <Button type="button" variant="outline" size="sm" className="mt-5 border-border bg-transparent text-xs text-foreground" disabled={!canEdit} onClick={setDefault}><Check className="mr-2 h-4 w-4" />Set as default route</Button>}
            </section>
          </div>
          ...
          <footer className="flex items-center justify-between border-t border-border px-5 py-4">
            ...
            <span className="text-[11px] text-muted-foreground">{routes.length} route{routes.length === 1 ? '' : 's'} configured</span>
          </footer>
        </section>
      </div>
    </main>
...
function RouteCanvasSkeleton() {
  return <div className="min-h-full bg-background p-4"><div className="mx-auto grid min-h-[720px] max-w-[1120px] overflow-hidden rounded-lg border border-border bg-card lg:grid-cols-[250px_1fr]"><aside className="space-y-3 border-r border-border p-4"><Skeleton className="h-5 w-28"/><Skeleton className="h-9 w-full"/><Skeleton className="h-9 w-full"/><Skeleton className="mt-6 h-20 w-full"/><Skeleton className="h-20 w-full"/></aside><main className="space-y-5 p-5"><Skeleton className="h-14 w-full"/><Skeleton className="h-56 w-full"/><div className="grid gap-4 md:grid-cols-2"><Skeleton className="h-72 w-full"/><Skeleton className="h-72 w-full"/></div></main></div></div>;
}
