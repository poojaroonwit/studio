...
| **When** | Real-time tracking during the lifecycle of every candidate. |
| **Why** | To maintain high service standards and ensure candidates don't "drop out" due to slow response times. |
...
## 1. Critical Metrics
- **SLA (Service Level Agreement)**: The target time allowed for a candidate to stay in a stage (e.g., Screening should take < 48 hours).
...
- **Overdue Tasks**: Dashboard widgets will prioritize actions for candidates who have been waiting the longest.

## 3. How to Verify (Test Case)
To test SLA visualization:
1.  **Navigate**: Find a candidate card on the **Kanban board** that has been in a stage for less than 1 hour.
