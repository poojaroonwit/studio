...
const patterns = {
  ...
  clientVar: /(?:const|let|var)\s+(\w+)\s*=\s*await\s+getPool\(\)\.connect\(\)/g,
  // Client release - more flexible pattern
  clientRelease: /(\w+)\.release\(\)/g,
  // Try-finally pattern - more flexible
  tryFinally: /try\s*\{[\s\S]*?\}\s*finally\s*\{[\s\S]*?\}/g,
  // WithDbClient usage
...
function analyzeFile(filePath, results) {
  try {
    ...
    if (connectMatches.length > 0) {
      ...
      // Extract client variable names from connect calls
      const clientVarMatches = [...content.matchAll(patterns.clientVar)];
      clientVarMatches.forEach(match => {
        if (match[1]) {
          fileResult.clientVariables.add(match[1]);
        }
      });
      ...
      // Check if file has client.release() for any client variable
      const hasAnyRelease = Array.from(fileResult.clientVariables).some(clientVar => {
        const releasePattern = new RegExp(`${clientVar}\\.release\\(\\)`, 'g');
        return releasePattern.test(content);
      });
      ...
      // Check if file has proper try-finally pattern around client operations
      const hasProperTryFinally = Array.from(fileResult.clientVariables).some(clientVar => {
        // Look for try-finally blocks that contain the client variable
        const tryFinallyPattern = new RegExp(
          `try\\s*\\{[\\s\\S]*?${clientVar}\\.[\\s\\S]*?\\}\\s*finally\\s*\\{[\\s\\S]*?${clientVar}\\.release\\(\\)[\\s\\S]*?\\}`,
          'g'
        );
        return tryFinallyPattern.test(content);
      });
      
      if (hasProperTryFinally) {
      ...
      if (fileResult.connectCalls.length > 0) {
        ...
        } else if (!fileResult.hasTryFinally && fileResult.hasRelease && !fileResult.usesWrapper) {
        ...
        } else if (fileResult.hasRelease) {
          fileResult.issues.push('✅ Has client release but no try-finally');
        }
      }
...
function printResults(results) {
  console.log('🔍 Database Connection Leak Analysis (Improved)\n');
