...
import { CheckIcon as Check, ChevronDownIcon as ChevronDown, ArrowPathIcon as Loader2, UserIcon as User, UserPlusIcon as UserPlus, UserMinusIcon as UserX, MagnifyingGlassIcon as Search, XMarkIcon as X } from '@heroicons/react/24/outline';
...
export function ApplicantRecruiterCell({
  ...
  const currentRecruiter = availableRecruiter.find(r => r.id === applicant.recruiterId);

  // Get the correct recruiter data - prefer applicant.recruiter if available, otherwise fall back to availableRecruiter
  const displayRecruiter = applicant.recruiter || currentRecruiter;
  ...
  }, [isAssigning, applicant.id, onResetAssigning]);
