...
type CreationSource = "direct" | "applicant";
type DirectStep = "core" | "work" | "personal" | "compliance" | "review";

const directSteps: Array<{ id: DirectStep; label: string; description: string }> = [
  { id: "core", label: "Core record", description: "Required identity" },
  { id: "work", label: "Work assignment", description: "Role and reporting" },
  { id: "personal", label: "Personal & contact", description: "Contact information" },
...
function HorizontalFieldRow({
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-[170px_minmax(0,1fr)] sm:items-center">
      <div>
        <Label htmlFor={htmlFor}>{label}{required && <span className="text-red-600"> *</span>}</Label>
        {hint && <p className="mt-1 text-xs leading-4 text-slate-500 dark:text-slate-400">{hint}</p>}
      </div>
      {children}
    </div>
  );
