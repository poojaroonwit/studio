...
| **When** | When the AI is consistently over-scoring or under-scoring candidates for a specific department. |
...
- `{{candidate_name}}`: Replaced by the name on the resume.
...
1.  **Navigate**: Create an "Evaluation" prompt that starts with: "Identify if {{candidate_name}} is good for {{position_title}}".
2.  **Act**: Go to a candidate profile and click **"Generative AI"**.
3.  **Confirm**: Select your new prompt and click **"Generate"**. The AI response should correctly name the candidate and the job title in its analysis.
