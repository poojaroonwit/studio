...
interface AddApplicantModalProps {
  ...
  onAddApplicant: (data: AddApplicantFormValues) => Promise<void>;
  availableStages: RecruitmentStage[]; // Remove availablePositions prop
}

const PLACEHOLDER_VALUE_NONE = "___NOT_SPECIFIED___";

export function AddApplicantModal({ isOpen, onOpenChange, onAddApplicant, availableStages }: AddApplicantModalProps) {
  ...
  const onSubmit = async (data: AddApplicantFormValues) => {
    try {
      ...
      await onAddApplicant(processedData);
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col p-0 overflow-hidden" dialogId="add-applicant-modal">
        <DialogHeader className="p-6 pb-0 flex-shrink-0">
          <DialogTitle className="flex items-center"><UserPlus className="mr-2 h-6 w-6 text-primary" /> Add New Applicant</DialogTitle>
          <DialogDescription>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        >
                          ...
                          <SelectContent selectId="add-applicant-start-month-select">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          >
                            ...
                            <SelectContent selectId="add-applicant-end-month-select">
                              {Array.from({ length: 12 }, (_, i) => i + 1).map(month => (
                                <SelectItem key={month} value={month.toString()}>
                                  {new Date(2000, month - 1).toLocaleDateString('en-US', { month: 'long' })}
                                </SelectItem>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        >
                          ...
                          <SelectContent selectId="add-applicant-education-start-month-select">
                            {Array.from({ length: 12 }, (_, i) => i + 1).map(month => (
                              <SelectItem key={month} value={month.toString()}>
                                {new Date(2000, month - 1).toLocaleDateString('en-US', { month: 'long' })}
                              </SelectItem>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          >
                            ...
                            <SelectContent selectId="add-applicant-education-end-month-select">
                              {Array.from({ length: 12 }, (_, i) => i + 1).map(month => (
                                <SelectItem key={month} value={month.toString()}>
                                  {new Date(2000, month - 1).toLocaleDateString('en-US', { month: 'long' })}
                                </SelectItem>
