...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const invalidApplicants = validationResults.filter(r => !r.valid);
    if (invalidApplicants.length > 0) {
      return NextResponse.json({
        error: 'Validation failed',
        details: invalidApplicants.map(r => ({
        ...
        try {
          ...
          if (applicant.id) {
            ...
            const updateResult = await client.query(updateQuery, [
              ...
              fitScore,
              applicant.statusId || 'Applied', // Use statusId if provided, otherwise default to 'Applied'
          ...
          results.errors.push(`Failed to process ${applicant.email}: ${(error as Error).message}`);
        }
      }

      await client.query('COMMIT');

      await logAudit('AUDIT', `Applicants imported by ${actingUserName}. Created: ${results.created}, Updated: ${results.updated}, Errors: ${results.errors.length}`, 'API:Applicants:Import', actingUserId, {
        created: results.created,
        updated: results.updated,
        errors: results.errors.length,
        totalProcessed: validApplicants.length,
        totalRows: Applicants.length
      });
    ...
    await logAudit('ERROR', `Failed to import Applicants by ${actingUserName}. Error: ${(error as Error).message}`, 'API:Applicants:Import', actingUserId, {
      error: (error as Error).message
    });
    return NextResponse.json({ error: 'Failed to import Applicants', details: (error as Error).message }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const templateData = [
      {
        'ID': '', // Leave blank for new Applicants, or provide existing ID for updates
        'Name*': '', // Required: Full name of the Applicant
        ...
        'Status*': 'Applied', // Required: Applicant status (Applied, Interviewing, Hired, etc.)
        ...
        'Location': '', // Optional: Applicant location
        'Introduction/About Me': '', // Optional: Applicant introduction or about section
    ...
    */
    ...
    const instructionsData = [
      ...
      { 'Field': 'Status*', 'Required': 'Yes', 'Description': 'Applicant status (Applied, Interviewing, Hired, etc.)' },
