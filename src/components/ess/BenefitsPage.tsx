...
type Enrollment = { id: string; benefitPlanId: string; status: string; version: number; effectiveFrom?: string | null; effectiveTo?: string | null; endedAt?: string | null; evidence?: unknown; benefitPlan: Plan };
type Payload = { employee: { name: string }; plans: Plan[]; enrollments: Enrollment[] };
type Dependent = { name: string; relationship: string };

type BenefitDecisionNote = { action?: string; comment?: string; createdAt?: string; toStatus?: string };
...
function latestDecisionNote(evidence: unknown): BenefitDecisionNote | null {
...
export function BenefitsPage() {
  ...
  const [pendingAction, setPendingAction] = React.useState<{ enrollment: Enrollment; action: Extract<BenefitAction, 'withdraw' | 'request_termination'> } | null>(null);
  ...
  return <main className="min-h-full bg-[hsl(var(--app-page-background,var(--background)))] px-3 py-4 text-foreground sm:px-5 lg:px-7"><div className="mx-auto max-w-[1440px] space-y-4">
    ...
    {loading ? <p className="py-16 text-center text-sm text-muted-foreground">Loading available benefits…</p> : !data ? <p className="py-16 text-center text-sm text-destructive">Benefits are currently unavailable.</p> : <>
      ...
      {data.enrollments.length ? <section className="mt-9"><h2 className="text-lg font-semibold">Application & coverage history</h2><div className="mt-3 divide-y divide-border border-y border-border bg-background">{data.enrollments.map(item => { const actions = employeeBenefitActions(item.status).filter((action): action is Extract<BenefitAction, 'withdraw' | 'resubmit' | 'request_termination'> => ['withdraw', 'resubmit', 'request_termination'].includes(action)); const decision = latestDecisionNote(item.evidence); return <div key={item.id} className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="font-semibold">{item.benefitPlan?.name || 'Benefit plan'}</p><p className="mt-1 text-xs text-muted-foreground">Coverage from {item.effectiveFrom ? new Date(item.effectiveFrom).toLocaleDateString() : 'approval date'}{item.effectiveTo ? ` · through ${new Date(item.effectiveTo).toLocaleDateString()}` : ''}</p>{decision?.comment ? <div className="mt-2 rounded-md border border-border bg-muted/30 px-3 py-2"><p className="text-xs font-semibold capitalize">Reviewer note · {String(decision.action || decision.toStatus || 'decision').replace(/_/g, ' ')}</p><p className="mt-1 text-sm text-muted-foreground">{decision.comment}</p></div> : null}</div><div className="flex flex-wrap items-center gap-2 sm:justify-end"><Badge variant={item.status === 'active' ? 'success' : 'secondary'}>{item.status.replace(/_/g, ' ')}</Badge>{actions.map(action => <Button key={action} size="sm" variant={action === 'request_termination' ? 'outline' : 'secondary'} disabled={actionBusy === item.id} onClick={() => action === 'resubmit' ? void act(item, action) : setPendingAction({ enrollment: item, action })}>{actionBusy === item.id ? 'Updating…' : actionLabel[action]}</Button>)}</div></div>; })}</div></section> : null}
    </>}
    ...
    <EssConfirmActionDialog
      ...
      title={pendingAction?.action === 'request_termination' ? 'Request coverage termination?' : 'Withdraw benefit application?'}
      description={pendingAction?.action === 'request_termination'
        ? 'Coverage stays active until the termination request is reviewed and approved.'
        : 'This removes the application from review. You can resubmit later only when the lifecycle allows it.'}
      confirmLabel={pendingAction?.action === 'request_termination' ? 'Request termination' : 'Withdraw application'}
      destructive
      busy={Boolean(actionBusy)}
      onConfirm={() => {
        const pending = pendingAction;
        setPendingAction(null);
        if (pending) void act(pending.enrollment, pending.action);
      }}
    />
  </div></main>;
}
