...
export function PayrollApprovalRoutesClient({ canEdit }: { canEdit: boolean }) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const response = await fetch('/api/settings/payroll-approval-routes', { cache: 'no-store' });
        if (!response.ok) throw new Error('Failed to load payroll approval routes');
        ...
        toast.error(error instanceof Error ? error.message : 'Failed to load payroll approval routes');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.message || 'Failed to save payroll approval routes');
      if (payload.routes) setRoutes(payload.routes);
      toast.success('Payroll approval routes saved');
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to save payroll approval routes');
    } finally {
      setSaving(false);
    }
  };
    ...
    <main className="min-h-full bg-background dark:bg-[#0b1119] p-3 text-foreground dark:text-[#e8edf4] sm:p-4">
      <div className="mx-auto grid min-h-[720px] max-w-[1120px] overflow-hidden rounded-[6px] border border-border dark:border-[#273240] bg-card dark:bg-[#101821] lg:grid-cols-[250px_minmax(0,1fr)]">
        <aside className="border-b border-border dark:border-[#273240] bg-muted/40 dark:bg-[#0d151e] lg:border-b-0 lg:border-r">
          ...
          <nav aria-label="Payroll approval routes" className="divide-y divide-border dark:divide-[#273240]">
        ...
        <section className="min-w-0">
          <header className="flex flex-col gap-4 border-b border-border dark:border-[#273240] px-5 py-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-muted-foreground dark:text-[#8d9caf]">Configure the ordered reviews applied when a payroll run is submitted.</p>
            </div>
          ...
          <section className="border-b border-border dark:border-[#273240] px-5 py-5">
            <div className="flex items-end justify-between gap-4">
              <div>
                <h3 className="text-sm font-semibold text-foreground dark:text-white">Approval sequence</h3>
                <p className="mt-1 text-xs text-muted-foreground dark:text-[#8d9caf]">Approvals occur in the order shown.</p>
              </div>
            </div>
