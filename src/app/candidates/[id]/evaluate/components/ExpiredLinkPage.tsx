...
export function ExpiredLinkPage({
  ...
  const handleReactivate = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/candidates/${candidateId}/evaluation-link`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.token) {
          window.location.href = `/candidates/${candidateId}/evaluate?token=${data.token}`;
        } else {
      ...
      <div className="py-8 flex items-center justify-between px-6 sm:px-10">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-semibold leading-tight" style={{ color: `hsl(${evaluateHeaderTextColor})` }}>
            {candidateName || 'Candidate Evaluation'}
          </h1>
        </div>
        {appLogoUrl && (
          <div>
