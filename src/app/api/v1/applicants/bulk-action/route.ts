...
export async function POST(req: NextRequest) {
  ...
  const { action, applicantIds, data } = validationResult.data;
  ...
  try {
    ...
    if (applicantsWithoutPermission.length > 0) {
      ...
      const deniedApplicants = applicantsWithoutPermission.map(c => (c as any).applicantId).join(', ');
    ...
    const applicantIdsWithPermission = applicantsWithPermission.map(c => c.id);
    ...
    switch (action) {
      ...
      default:
        await client.query('ROLLBACK');
        await logAudit('ERROR', `Bulk action failed (invalid action) by ${getActingUserName(user)}.`, 'API:V1:Applicants:BulkAction', user.id, { action, applicantIds });
    ...
    if (action === 'assign_position' && data?.positionId) {
      try {
        // console.log(`Bulk assigning position ${data.positionId} to ${applicantIdsWithPermission.length} Applicants`);
    ...
    await logAudit('AUDIT', `Bulk action '${action}' performed by ${getActingUserName(user)}. Affected: ${result.rowCount}.`, 'API:V1:Applicants:BulkAction', user.id, { action, applicantIds, data, affectedCount: result.rowCount });
    return new Response(JSON.stringify({
    ...
    await client.query('ROLLBACK');
    await logAudit('ERROR', `Bulk action '${action}' failed by ${getActingUserName(user)}. Error: ${(error as Error).message}`, 'API:V1:Applicants:BulkAction', user.id, { action, applicantIds, data, error: (error as Error).message });
