...
const EMPTY_FILTERS: ApplicantExportFilters = {
  ...
  transferDomains: 'people,recruiting,attendance,leave,performance,learning,payroll,surveys,expenses,configuration',
};

const TRANSFER_DOMAINS = [
  ['people', 'People & organization'], ['recruiting', 'Recruiting'], ['attendance', 'Attendance & shifts'],
  ['leave', 'Leave'], ['performance', 'Performance'], ['learning', 'Learning'], ['payroll', 'Payroll & compensation'],
  ['surveys', 'Surveys'], ['expenses', 'Expenses & travel'], ['configuration', 'Business configuration'],
] as const;

export function DataOperationsWorkspace() {
  const [tab, setTab] = React.useState<WorkspaceTab>('import');
  ...
  const selectModel = (id: DataOperationModelId) => {
    ...
    if (id === 'system-transfer') setFormat('zip');
  };
  ...
  const hasTransferDomains = modelId !== 'system-transfer' || filters.transferDomains.split(',').filter(Boolean).length > 0;
      ...
      <header className="border-b bg-background/95 px-4 pt-5 sm:px-6 lg:px-8">
        <div className="w-full">
          <div className="pb-5">
            ...
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Move recruiting data with a guided, review-before-run workflow.</p>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
            <div className="min-w-0">
              ...
              <div className="mt-5 overflow-hidden rounded-xl border bg-background">
                ...
                {step === 2 && tab === 'import' && (
                  <ImportConfigureStep model={model} file={file} isDownloading={isDownloadingTemplate} onDownload={handleTemplateDownload} onFile={handleFile} />
                )}
                {step === 2 && tab === 'export' && (
...
function StepRail({ current, mode }: { current: number; mode: 'import' | 'export' }) {
  ...
  return <ol className="grid grid-cols-3" aria-label={`${mode} progress`}>{labels.map((label, index) => {
    const number = index + 1;
    const complete = number < current;
    const active = number === current;
    return <li key={label} className="relative flex items-center gap-2 after:absolute after:left-[calc(50%+1.25rem)] after:right-[calc(-50%+1.25rem)] after:top-4 after:h-px after:bg-border last:after:hidden">
      <span className={cn('relative z-10 grid h-8 w-8 shrink-0 place-items-center rounded-full border bg-background text-xs font-semibold text-muted-foreground', active && 'border-primary bg-primary text-primary-foreground', complete && 'border-primary bg-primary/10 text-primary')}>{complete ? <Check className="h-4 w-4" /> : number}</span>
...
function ChooseDataStep({ selected, onSelect }: { selected: DataOperationModelId; onSelect: (id: DataOperationModelId) => void }) {
  ...
  const models = DATA_OPERATION_MODELS.filter((model) => model.id !== 'system-transfer' || session?.user?.role === 'Admin');
  return <StepContent eyebrow="Step 1" title="What data are you moving?" description="Only production-backed data types are shown here.">
    <div className="grid gap-3 sm:grid-cols-2">{models.map((model) => {
      const Icon = model.id === 'applicants' ? Users : model.id === 'positions' ? BriefcaseBusiness : Database;
      const active = selected === model.id;
      return <button key={model.id} type="button" onClick={() => onSelect(model.id)} aria-pressed={active} className={cn('group flex min-h-36 flex-col items-start rounded-xl border p-5 text-left transition-colors hover:bg-muted/35 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', active && 'border-primary bg-primary/[.04] ring-1 ring-primary/20')}>
        <span className={cn('grid h-9 w-9 place-items-center rounded-lg bg-muted text-muted-foreground', active && 'bg-primary/10 text-primary')}><Icon className="h-4.5 w-4.5" /></span>
        <span className="mt-4 flex w-full items-center justify-between gap-3"><span className="font-semibold">{model.name}</span>{active && <CheckCircle2 className="h-5 w-5 text-primary" />}</span>
        <span className="mt-1 text-sm leading-5 text-muted-foreground">{model.description}</span>
        <span className="mt-3 text-xs font-medium text-muted-foreground">Import {model.importTypesLabel} · Export {model.exportFormatsLabel}</span>
      </button>;
    })}</div>
  </StepContent>;
}
...
function ImportConfigureStep({ model, file, isDownloading, onDownload, onFile }: { model: DataOperationModel; file: File | null; isDownloading: boolean; onDownload: () => void; onFile: (file: File | null) => void }) {
  ...
  return <StepContent eyebrow="Step 2" title={`Prepare your ${model.name.toLowerCase()} file`} description="Start from the template so headers and values match the importer.">
    <div className="flex flex-col justify-between gap-4 rounded-lg bg-muted/35 p-4 sm:flex-row sm:items-center">
      ...
      <Button variant="outline" size="sm" onClick={onDownload} disabled={isDownloading || model.id === 'system-transfer'}>{isDownloading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ArrowDownToLine className="mr-2 h-4 w-4" />}{model.id === 'system-transfer' ? 'Export to create package' : 'Download template'}</Button>
    </div>
    <div className="mt-5">
      <Label>Completed file</Label>
      <div onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { event.preventDefault(); if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragging(false); }} onDrop={(event) => { event.preventDefault(); setDragging(false); onFile(event.dataTransfer.files?.[0] ?? null); }} className={cn('mt-2 flex min-h-52 flex-col items-center justify-center rounded-xl border border-dashed bg-muted/15 px-6 text-center transition-colors', dragging && 'border-primary bg-primary/[.04]', file && 'border-emerald-500/40 bg-emerald-500/[.04]')}>
        <input ref={inputRef} type="file" accept={model.accept} className="sr-only" onChange={(event) => onFile(event.target.files?.[0] ?? null)} />
        {file ? <><span className="grid h-11 w-11 place-items-center rounded-full bg-emerald-500/10 text-emerald-600"><Check className="h-5 w-5" /></span><p className="mt-3 max-w-full truncate text-sm font-medium">{file.name}</p><p className="mt-1 text-xs text-muted-foreground">{formatBytes(file.size)} · ready to review</p><Button type="button" variant="ghost" size="sm" className="mt-3" onClick={() => { onFile(null); if (inputRef.current) inputRef.current.value = ''; }}><X className="mr-2 h-3.5 w-3.5" /> Remove</Button></> : <><span className="grid h-11 w-11 place-items-center rounded-full bg-primary/10 text-primary"><UploadCloud className="h-5 w-5" /></span><p className="mt-3 text-sm font-medium">Drop your file here</p><p className="mt-1 text-xs text-muted-foreground">{model.uploadHelp}</p><Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => inputRef.current?.click()}>Browse files</Button></>}
      </div>
    </div>
  </StepContent>;
}

function ExportConfigureStep({ model, format, filters, onFormat, onFilters }: { model: DataOperationModel; format: ExportFormat; filters: ApplicantExportFilters; onFormat: (value: ExportFormat) => void; onFilters: (value: ApplicantExportFilters) => void }) {
  const update = (key: keyof ApplicantExportFilters, value: string) => onFilters({ ...filters, [key]: value });
  return <StepContent eyebrow="Step 2" title="Configure the export" description={model.id === 'applicants' ? 'Narrow the export when needed. Blank filters include every applicant you can access.' : model.id === 'positions' ? 'The positions exporter includes all position fields and records.' : 'Choose the business domains to include in the validated ZIP transfer package.'}>
    <div className="max-w-sm"><Label>File format</Label><Select value={format} onValueChange={(value) => onFormat(value as ExportFormat)} disabled={model.id !== 'applicants'}><SelectTrigger className="mt-2"><SelectValue /></SelectTrigger><SelectContent>{model.id === 'system-transfer' ? <SelectItem value="zip">HRI transfer package (.zip)</SelectItem> : <SelectItem value="excel">Excel workbook (.xlsx)</SelectItem>}{model.id === 'applicants' && <SelectItem value="csv">CSV file (.csv)</SelectItem>}</SelectContent></Select></div>
    {model.id === 'applicants' ? <div className="mt-6 border-t pt-5"><div className="flex items-center justify-between"><div><h3 className="text-sm font-semibold">Optional filters</h3><p className="mt-0.5 text-xs text-muted-foreground">All filled filters are applied together.</p></div><Button variant="ghost" size="sm" onClick={() => onFilters(EMPTY_FILTERS)}>Clear</Button></div><div className="mt-4 grid gap-4 sm:grid-cols-2">
    ...
    </div></div> : model.id === 'system-transfer' ? <TransferDomainSelector value={filters.transferDomains} onChange={(transferDomains) => onFilters({ ...filters, transferDomains })} /> : <div className="mt-6 rounded-lg border bg-muted/25 p-4 text-sm text-muted-foreground">Positions are exported as a complete Excel workbook. Filtering and column selection are not supported.</div>}
  </StepContent>;
}
...
function ReviewStep({ mode, model, file, format, filters, result }: { mode: 'import' | 'export'; model: DataOperationModel; file: File | null; format: ExportFormat; filters: ApplicantExportFilters; result: ImportResult | null }) {
  ...
  return <StepContent eyebrow="Step 3" title={`Review your ${mode}`} description="Nothing runs until you confirm below."><div className="divide-y rounded-lg border">
    <ReviewRow label="Data" value={model.name} />
    {mode === 'import' ? <><ReviewRow label="File" value={file?.name ?? 'No file selected'} /><ReviewRow label="File size" value={file ? formatBytes(file.size) : '—'} /><ReviewRow label="Behavior" value={model.importBehavior} /></> : <><ReviewRow label="Format" value={format === 'excel' ? 'Excel workbook (.xlsx)' : format === 'zip' ? 'HRI transfer package (.zip)' : 'CSV file (.csv)'} /><ReviewRow label="Records" value={model.id === 'applicants' && activeFilters.filter(([key]) => key !== 'transferDomains').length ? `${activeFilters.filter(([key]) => key !== 'transferDomains').length} filters applied` : model.id === 'system-transfer' ? `${filters.transferDomains.split(',').filter(Boolean).length} business domains` : 'All accessible records'} /></>}
  </div><div className="mt-5 flex items-start gap-3 rounded-lg bg-primary/[.05] p-4"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-primary" /><div><p className="text-sm font-medium">Ready to add to queue</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{mode === 'import' ? model.reviewHelp : 'The export will be generated asynchronously. Download it from the data job queue when it is ready.'}</p></div></div></StepContent>;
}
...
function FlowSummary({ mode, step, model, file, format, filters }: { mode: 'import' | 'export'; step: number; model: DataOperationModel; file: File | null; format: ExportFormat; filters: ApplicantExportFilters }) {
  const filterCount = Object.entries(filters).filter(([key, value]) => key !== 'transferDomains' && Boolean(value)).length;
  const selectionLabel = model.id === 'system-transfer' ? `${filters.transferDomains.split(',').filter(Boolean).length} domains` : filterCount ? String(filterCount) : 'None';
  return <aside className="h-fit rounded-xl border bg-background p-5 lg:sticky lg:top-4"><p className="text-sm font-semibold">Current operation</p><div className="mt-4 space-y-3 text-sm"><ReviewRow compact label="Action" value={mode === 'import' ? 'Import' : 'Export'} /><ReviewRow compact label="Data" value={model.name} /><ReviewRow compact label="Progress" value={`Step ${step} of 3`} />{mode === 'import' ? <ReviewRow compact label="File" value={file ? file.name : 'Not selected'} /> : <><ReviewRow compact label="Format" value={format.toUpperCase()} /><ReviewRow compact label={model.id === 'system-transfer' ? 'Selection' : 'Filters'} value={selectionLabel} /></>}</div><p className="mt-5 border-t pt-4 text-xs leading-5 text-muted-foreground">Confirming creates a queued job. Processing never runs inside the submission request, and every affected record is audit logged.</p></aside>;
}
...
function TransferDomainSelector({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  const toggle = (domain: string) => {
    ...
    onChange(TRANSFER_DOMAINS.map(([id]) => id).filter((id) => next.has(id)).join(','));
  };
  return <div className="mt-6 border-t pt-5"><div><h3 className="text-sm font-semibold">Business domains</h3><p className="mt-1 text-xs text-muted-foreground">Sensitive authentication, security, audit, notification, and operational records are always excluded.</p></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{TRANSFER_DOMAINS.map(([id, label]) => <label key={id} className="flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-2.5 text-sm"><input type="checkbox" checked={selected.has(id)} onChange={() => toggle(id)} className="h-4 w-4 rounded border-input accent-primary" /><span>{label}</span></label>)}</div>{selected.size === 0 && <p className="mt-2 text-xs font-medium text-destructive">Select at least one domain.</p>}</div>;
}
...
function ReviewRow({ label, value, compact = false }: { label: string; value: string; compact?: boolean }) { return <div className={cn('flex items-start justify-between gap-4', compact ? 'border-b pb-3 last:border-0 last:pb-0' : 'px-4 py-3')}><span className="text-muted-foreground">{label}</span><span className="max-w-[65%] truncate text-right font-medium" title={value}>{value}</span></div>; }
