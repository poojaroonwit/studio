...
import { ApplicantAssetsSection } from './components/ApplicantAssetsSection';
...
export default function ApplicantEvaluationPage() {
  ...
  // Load shared remarks when Applicant data is available
  useEffect(() => {
    if (applicantData) {
      const sharedRemarks = applicantData?.customAttributes?.interviewRemarks ||
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (evaluationsMap.size > 0) {
          ...
          // Set remark text from Applicant customAttributes (shared remarks)
          // Load from Applicant data if available, otherwise use default
          const sharedRemarks = applicantData?.customAttributes?.interviewRemarks ||
            applicantData?.custom_attributes?.interviewRemarks ||
            '';
          setRemarkText(sharedRemarks);
          ...
          // Load shared remarks from Applicant data
          const sharedRemarks = applicantData?.customAttributes?.interviewRemarks ||
            applicantData?.custom_attributes?.interviewRemarks ||
            '';
          setRemarkText(sharedRemarks);
  ...
  const fetchEvaluationData = async () => {
    try {
      ...
      setPositionId(applicantPositionId);
      setPositionTitle(applicant.position?.title || null);

      const evaluationUrl = token
        ? `/api/v1/positions/${applicantPositionId}/evaluation?token=${encodeURIComponent(token)}`
        : `/api/v1/positions/${applicantPositionId}/evaluation`;
      ...
      // Load interviewers assigned to the Applicant's position
      try {
        const ivRes = await fetch(`/api/positions/${applicantPositionId}/interviewers`, { credentials: 'include' });
        if (ivRes.ok) {
      ...
      try {
        const sourcesRes = await fetch('/api/settings/Applicant-sources', { credentials: 'include' });
        if (sourcesRes.ok) {
      ...
      if (validQuestions.length === 0) {
        ...
        throw new Error(`No evaluation traits configured for ${positionName}. Please configure personality traits in the position settings before evaluating applicants.`)
      }
  ...
  const handleSubmitEvaluation = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/applicants/${candidateId}/evaluation`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          positionId: formData.applicant.positionId || undefined,
  ...
  const saveRemark = async (text: string) => {
    ...
    try {
      ...
      const currentCustomAttributes = applicantData?.customAttributes || applicantData?.custom_attributes || {};
    ...
    if (error === 'Permission denied') {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center" style={{ backgroundColor: sidebarBgColor || 'hsl(var(--background))' }}>
          ...
          <Button onClick={() => router.push('/applicants')} variant="outline">
            Back to Applicants
          </Button>
        </div>
      );
      ...
      <div className="py-6 flex items-center justify-between px-6 sm:px-10">
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div>
            ...
            <h1 className="text-xl sm:text-3xl font-semibold leading-tight" style={{ color: `hsl(${evaluateHeaderTextColor})` }}>{formData.applicant.name}</h1>
          </div>
        </div>
        {appLogoUrl && (
          <div>
