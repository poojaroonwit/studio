...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle fit score filters - same logic as main Applicants API
    ...
    if (minExperienceYears !== null) {
      ...
      if (minExp === -1) {
        ...
        // Include Applicants with no experience data OR Applicants with experience >= minExp
    ...
    if (maxExperienceYears !== null) {
      ...
      // Include Applicants with no experience data OR Applicants with experience <= maxExp
      whereClauses.push(`(c."parsedData"->>'totalExperienceYears' IS NULL OR CAST(c."parsedData"->>'totalExperienceYears' AS DECIMAL) <= $${paramIndex++})`);
    ...
    // Query to get all Applicants with both applied and matching scores
    const query = `
      SELECT 
