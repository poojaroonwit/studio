...
export function AppliedApplicantsTable({
  ...
  let rowNumber = 1;
  const pinned = applicants.filter(c => c.isPinned);
  const unpinned = applicants.filter(c => !c.isPinned);
  ...
  return (
    <Table containerClassName="overflow-visible">
      <TableHeader>
        <TableRow>
          <TableHead>#</TableHead>
          {visibleColumns.name && <SortableHeader column="name" label="Applicant" />}
          ...
          return (
            ...
            <TableCell>
              {applicant.expectedSalary ? (
                <span className="text-sm font-medium">฿{applicant.expectedSalary.toLocaleString()}</span>
              ) : (
          ...
          <TableRow className="bg-muted/30 border-b border-muted">
            <TableCell colSpan={Object.values(visibleColumns).filter(Boolean).length + 1} className="py-2 px-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">All Applicants</span>
