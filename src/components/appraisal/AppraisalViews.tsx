...
function GoalRow({ goal }: { goal: Row }) {
  const { t } = useLocalization();
  return (
...
function PulseRow({ label: itemLabel, value, helper, attention }: { label: string; value: number; helper: string; attention?: boolean }) {
...
function nextManagerInstruction(row: Row, t: (key: string, fallback: string) => string) {
...
function formatDate(value: unknown, t: (key: string, fallback: string) => string) {
...
function formatDateTime(value: unknown, t: (key: string, fallback: string) => string) {
...
function formatYear(value: unknown, t: (key: string, fallback: string) => string) {
  return value ? new Date(String(value)).getFullYear() : t('appraisal.shared.periodFallback', 'Period');
}

function stripIdempotency(value: string) {
