...
interface CandidatesPageMobileFitScoreFilterProps {
...
export function CandidatesPageMobileFitScoreFilter({
  selectedGrades,
  onGradeToggle,
  candidateCounts = [],
  filterMode = 'multi',
  ...
  const getTotalCount = (): number => {
    ...
    if (!Array.isArray(candidateCounts)) return 0;
    const total = candidateCounts.reduce((total, item) => total + (item?.count || 0), 0);
    return total;
  };
