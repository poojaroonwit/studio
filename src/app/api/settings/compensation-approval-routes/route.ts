...
import { approvalRouteCatalogSchema } from '@/lib/headcount-approval-path-config';
import {
  COMPENSATION_APPROVAL_ROUTES_SETTING_KEY,
  getCompensationApprovalRoutes,
} from '@/lib/compensation-approval-route-config';
...
export async function GET() {
  ...
  return NextResponse.json({ routes: await getCompensationApprovalRoutes() });
}
...
export async function PUT(request: NextRequest) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ message: parsed.error.issues[0]?.message || 'Invalid approval routes' }, { status: 400 });
  }
  ...
  await getPool().query(
    ...
    [COMPENSATION_APPROVAL_ROUTES_SETTING_KEY, JSON.stringify(parsed.data)],
  );
