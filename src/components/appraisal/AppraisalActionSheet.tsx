  ...
  return (
    <Sheet open={Boolean(mode)} onOpenChange={open => !open && onClose()}>
      <SheetContent side="right" className="w-full overflow-y-auto p-0 sm:max-w-2xl" sheetId="appraisal-action-sheet">
        {mode ? (
          <>
            <SheetHeader className="border-b border-border bg-muted/20 p-5 pr-14 text-left  ">
...
function ActionForm({
  ...
  return (
    <form onSubmit={submit} className="flex min-h-[calc(100dvh-7.5rem)] flex-col">
      ...
      <SheetFooter className="sticky bottom-0 gap-2 border-t border-border bg-background/95 p-4 backdrop-blur  ">
        <Button type="submit" disabled={saving || autosaveState === 'saving'} className="min-h-11 bg-primary text-primary-foreground hover:bg-primary/90">
  ...
  return (
    <div className="border border-border bg-muted/20 p-4  ">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-foreground ">{String(record.employeeName || record.name || t('appraisal.actionSheet.fallback.record', 'Appraisal record'))}</p>
    ...
    <>
      ...
      {goals.length ? (
        <div className="space-y-3">
          ...
          {goals.map(goal => (
            <div key={String(goal.id)} className="border border-border p-3 ">
  ...
  return (
    <>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="border border-border bg-muted/40 p-3 text-xs leading-5 text-muted-foreground   ">
  ...
  return (
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="flex min-h-12 items-center gap-3 border border-border px-3 text-sm "><input type="checkbox" checked={Boolean(form.requirePeerReview)} onChange={event => update('requirePeerReview', event.target.checked)} />{t('appraisal.actionSheet.fields.requirePeerReview.label', 'Require peer review')}</label>
        <label className="flex min-h-12 items-center gap-3 border border-border px-3 text-sm "><input type="checkbox" checked={Boolean(form.requireCalibration)} onChange={event => update('requireCalibration', event.target.checked)} />{t('appraisal.actionSheet.fields.requireCalibration.label', 'Require calibration')}</label>
      </div>
    </>
  );
}
...
function TemplateFields({ form, update }: FieldsProps) {
  ...
  return (
    <>
      ...
      <Field label={t('appraisal.actionSheet.fields.description.label', 'Description')}><Textarea value={String(form.description)} onChange={event => update('description', event.target.value)} className="min-h-24" /></Field>
      <div className="border border-warning/30 bg-warning/10 p-4 text-sm leading-6 text-foreground dark:border-amber-900/60 dark:bg-amber-950/20 dark:text-amber-100">
...
function AcknowledgmentFields({ form, update }: FieldsProps) {
  ...
  return (
    <>
      ...
      <label className="flex min-h-12 items-center gap-3 border border-border px-3 text-sm "><input type="checkbox" checked={Boolean(form.requestDiscussion)} onChange={event => update('requestDiscussion', event.target.checked)} />{t('appraisal.actionSheet.fields.requestDiscussion.label', 'Request a follow-up discussion instead')}</label>
    </>
  );
}
...
function ReviewerFields({ form, update, data }: FieldsProps & { data: AppraisalWorkspaceData }) {
  ...
  return (
    <>
      ...
      <label className="flex min-h-12 items-center gap-3 border border-border px-3 text-sm "><input type="checkbox" checked={Boolean(form.isAnonymous)} onChange={event => update('isAnonymous', event.target.checked)} />{t('appraisal.actionSheet.fields.protectReviewerIdentity.label', 'Protect reviewer identity from the employee')}</label>
    </>
  );
}
...
function Confirmation({ mode }: { mode: AppraisalActionMode }) {
  ...
  return <div className="border border-border bg-muted/40 p-4 text-sm leading-6 text-foreground/80   ">{text[mode]}</div>;
}
