      ...
      <div className="space-y-4">
        <WorkspaceSection title="Performance profile" description="One current view across Appraisal, Goal, feedback, competencies, and development.">
          ...
          <div className="mt-6 flex flex-wrap gap-2 border-t border-border/60 pt-5 ">
            <Button asChild className="bg-primary text-primary-foreground hover:bg-primary/90"><a href="/ess/performance">View appraisal</a></Button>
          ...
          {data.recognition.length ? (
            <div className="space-y-4">
              {data.recognition.slice(0, 6).map(item => (
                <article key={String(item.id)} className="border-b border-border/60 pb-4 last:border-0 last:pb-0 ">
                  <div className="flex items-center gap-2">
                    <Award className="h-4 w-4 text-amber-600" aria-hidden />
                    <p className="text-xs font-bold capitalize text-foreground ">{String(item.category || '').replace(/_/g, ' ')}</p>
                  </div>
                  <p className="mt-2 text-sm leading-6 text-muted-foreground ">{String(item.message)}</p>
...
export function CheckInsView({
  ...
  return (
    <WorkspaceSection
      ...
      action={<Button onClick={() => onAction('check-in')} className="bg-primary text-primary-foreground hover:bg-primary/90">Schedule check-in</Button>}
    >
      {data.checkIns.length ? (
        <div className="grid gap-3 lg:grid-cols-2">
          {data.checkIns.map(checkIn => (
            <article key={String(checkIn.id)} className="rounded-lg border border-border p-4 ">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-bold capitalize text-foreground ">{String(checkIn.type || 'Check-in').replace(/_/g, ' ')}</p>
              ...
              <p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground ">{String(checkIn.agenda || 'No agenda prepared.')}</p>
...
export function FeedbackView({
  ...
  return (
    <div className="space-y-4">
      <WorkspaceSection title="Continuous feedback" description="Visibility is enforced when records are read—not just hidden in the interface." action={<div className="flex gap-2"><Button variant="outline" onClick={() => onAction('feedback')}>Request feedback</Button><Button onClick={() => onAction('feedback')} className="bg-primary text-primary-foreground hover:bg-primary/90">Give feedback</Button></div>}>
        {requested.length ? (
        ...
        {published.length ? (
          <div className="grid gap-3 md:grid-cols-2">
            {published.map(item => (
              <article key={String(item.id)} className="rounded-lg border border-border p-4 ">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-bold capitalize text-foreground ">{String(item.feedbackType || 'Feedback').replace(/_/g, ' ')}</p>
        ...
        {competencies.length ? (
          <div className="space-y-4">
            {competencies.map(item => {
              ...
              return (
                <article key={item.name} className="grid gap-4 rounded-lg border border-border p-4  md:grid-cols-[minmax(0,1fr)_100px_100px_100px] md:items-center">
                  <div>
                    ...
                    <Progress value={Math.min(100, (item.currentLevel / Math.max(item.expectedLevel, 1)) * 100)} className="mt-3 h-1.5 bg-muted [&>div]:bg-emerald-600 " />
                  </div>
        ...
        {data.competencyEvidence.length ? (
          <div className="grid gap-3 md:grid-cols-2">
            {data.competencyEvidence.map(item => (
              <article key={String(item.id)} className="flex items-start gap-3 rounded-lg bg-muted/40 p-4 ">
                <FileCheck2 className="mt-0.5 h-5 w-5 shrink-0 text-primary dark:text-blue-300" aria-hidden />
                <div>
                  <p className="text-sm font-bold text-foreground ">{String(item.title)}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{String(item.competencyName)} · {String(item.evidenceType || '').replace(/_/g, ' ')}</p>
                  <p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground ">{String(item.description || 'No description supplied.')}</p>
...
export function DevelopmentView({
  ...
  return (
    <div className="space-y-4">
      <WorkspaceSection title="Individual development plans" description="Plans translate recorded needs into concrete actions while Learning remains the course source." action={<Button onClick={() => onAction('development')} className="bg-primary text-primary-foreground hover:bg-primary/90">Add development action</Button>}>
        {data.developmentPlans.length ? (
          <div className="space-y-3">
            {data.developmentPlans.map(plan => (
              <article key={String(plan.id)} className="rounded-lg border border-border p-4 ">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-sm font-bold text-foreground ">{String(plan.title)}</p>
                ...
                {plan.aspiration ? <p className="mt-3 text-sm leading-6 text-muted-foreground ">{String(plan.aspiration)}</p> : null}
                <div className="mt-4 space-y-2">
                  {data.developmentActions.filter(action => action.planId === plan.id).map(action => (
                    <button key={String(action.id)} type="button" className="flex w-full items-center gap-3 rounded-lg bg-muted/40 p-3 text-left transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring  dark:hover:bg-slate-900" onClick={() => onUpdate(action)}>
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold text-foreground ">{String(action.title)}</p>
        ...
        <div className="space-y-8">
          {Object.entries(years).sort(([a], [b]) => b.localeCompare(a)).map(([year, items]) => (
            <section key={year} className="grid gap-4 md:grid-cols-[100px_minmax(0,1fr)]">
              <h3 className="text-2xl font-bold tracking-[-0.04em] text-muted-foreground/50 dark:text-foreground/80">{year}</h3>
...
export function TeamPerformanceView({
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-px overflow-hidden rounded-lg border border-border bg-muted   sm:grid-cols-2 lg:grid-cols-4">
        ...
        {data.team.length ? (
          <>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {data.team.map(row => (
                    <tr key={String(row.id)} className="cursor-pointer transition-colors hover:bg-muted/40 dark:hover:bg-slate-900/50" onClick={() => setDetail(row)}>
                      ...
                      <td className="px-3 py-4 font-bold tabular-nums text-foreground ">{Number(row.overdueCheckIns || 0) + Number(row.overdueDevelopmentActions || 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="grid gap-3 md:hidden">
              {data.team.map(row => (
                <button key={String(row.id)} type="button" className="rounded-lg border border-border p-4 text-left " onClick={() => setDetail(row)}>
            ...
            <>
              ...
              <div className="space-y-5 p-5">
                ...
                <div className="rounded-lg border border-border p-4 ">
                  <p className="text-sm font-bold text-foreground ">Manager workspace</p>
                  ...
                  <Button className="mt-4 w-full bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => { onSelectEmployee(String(detail.id)); setDetail(null); }}>Open performance detail</Button>
                </div>
