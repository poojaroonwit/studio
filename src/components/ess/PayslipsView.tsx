...
function periodLabel(item: Payslip) {
  const date = new Date(String(item.published_at || item.created_at || ''));
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        <header className="flex items-end justify-between gap-4 border-b border-border pb-5">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">View published payroll statements through your secure employee account.</p>
          </div>
        ...
        {loading ? (
          ...
          <section className="rounded-lg border border-dashed border-border bg-card p-10 text-center">
            ...
            <h2 className="mt-3 font-semibold">No published payslips yet</h2>
            <p className="mt-1 text-sm text-muted-foreground">Payroll statements will appear here once Payroll publishes them.</p>
          </section>
        ) : (
          <section className="overflow-hidden rounded-lg border border-border bg-card" aria-label="Published payslips">
                ...
                <article key={item.id} className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center">
                  <div className="flex min-w-0 flex-1 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">Published {item.published_at ? new Date(item.published_at).toLocaleDateString() : 'date unavailable'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <StatusBadge status={item.status || 'published'} />
                    <Button asChild variant="outline" className="min-h-11">
                      <a href={`/api/ess/files?kind=payslip&id=${encodeURIComponent(item.id)}`} target="_blank" rel="noreferrer">
