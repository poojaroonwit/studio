...
const SearchApplicantsOutputSchema = z.object({
  matchedcandidateIds: z.array(z.string()).describe("An array of UUIDs of Applicants that match the search query."),
...
// Enhanced helper to create a more comprehensive summary for a Applicant
...
export async function searchApplicantsAIChat(input: SearchApplicantsInput): Promise<SearchApplicantsOutput> {
  ...
  try {
    ...
    if (filteredApplicants.length === 0) {
      return { matchedcandidateIds: [], aiReasoning: "No Applicants found in the database to search.", recordCount: 0 };
    }
  } catch (dbError) {
    return { matchedcandidateIds: [], aiReasoning: "Failed to retrieve Applicant data for searching.", recordCount: 0 };
  }
...
Do not include any markdown formatting, code blocks, or additional text. Only return the JSON object.`;
    ...
    if (input.query && effectiveApplicantData !== "No Applicant details available for processing." && (result.matchedcandidateIds || []).length === 0 && (!finalReasoning || finalReasoning.trim() === '')) {
