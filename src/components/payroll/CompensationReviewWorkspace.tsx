...
import { MetricStrip, Money, PayrollEmpty, PayrollStatus, SectionHeading } from './PayrollPrimitives';

type Row = Record<string, unknown>;
type CompTab = 'packages' | 'changes';
type ChangeType = 'salary_increase' | 'promotion' | 'merit' | 'market' | 'cost_of_living' | 'allowance' | 'bonus' | 'incentive' | 'retention' | 'correction';
type CompensationPayloadAction = 'submit_change' | 'approve_change' | 'reject_change';
...
const formatDate = (value: unknown) => {
  ...
  return Number.isNaN(date.getTime()) ? asString(value) : date.toLocaleDateString('en-GB');
};
...
function packageCurrency(records: CompensationPackageRow[], employeeId: string) {
  const packageRow = records.find(record => asString(record.employee_id) === employeeId);
...
export function CompensationReviewWorkspace({
  ...
  mutate,
  busy,
}: { data: PayrollWorkspacePayload; mutate: (body: Row, key: string) => Promise<unknown>; busy: string }) {
  ...
  const employees = React.useMemo<CompensationEmployeeRow[]>(() => {
    const rows = data.employees.length ? data.employees : data.records.map(record => ({
    ...
    return [...byId.values()].sort((a, b) => employeeDisplayName(a).localeCompare(employeeDisplayName(b)));
  ...
  const packages = React.useMemo<CompensationPackageRow[]>(() => {
    ...
    return normalized.filter(record => {
      const rowText = `${record.employee_name} ${record.employee_number} ${record.status}`.toLowerCase();
      return rowText.includes(query.toLowerCase());
    });
  }, [data.records, query]);

  const filteredPackages = React.useMemo(() => {
    const base = showInactivePackages ? packages : packages.filter(isActivePackage);
  ...
  const changes = React.useMemo<CompensationChangeRow[]>(() => data.secondary
    .map(normalizeChangeRow)
    .filter(record => (
      `${record.employee_name} ${record.employee_number} ${record.change_type} ${record.status}`.toLowerCase()
  ...
  const packageSummary = React.useMemo(() => {
    ...
    const pendingChanges = changes.filter(change => change.status === 'pending_approval');
    return {
  ...
  const openCreateForm = (record?: CompensationPackageRow) => {
    if (!canManage) return;
    const employeeId = asString(record?.employee_id || employees[0]?.id || data.records[0]?.employee_id);
    const selectedPackage = packages.find(item => asString(item.employee_id) === employeeId);
    const selectedEmployee = employees.find(item => asString(item.id) === employeeId);
    ...
    setForm({
      ...
      effectiveDate: defaultEffectiveDate(),
      reason: selected ? `Review compensation for ${asString(selected.employee_name || selected.name, 'employee')}` : 'Compensation change requested',
    });
  ...
  const handleCreateChange = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.proposedAmount || Number.isNaN(Number(form.proposedAmount)) || Number(form.proposedAmount) <= 0) {
  ...
  const handleReviewAction = async (row: CompensationChangeRow, action: CompensationPayloadAction) => {
  ...
  return (
    <div className="space-y-4 pb-8">
      <SectionHeading
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            {canManage && <Button onClick={() => openCreateForm()} className="h-9 bg-blue-600 text-white hover:bg-blue-500">New compensation change</Button>}
            {showCreateForm && <Button variant="outline" className="h-9 border-border bg-transparent" onClick={() => setShowCreateForm(false)}>Close form</Button>}
          </div>
        }
      />
      ...
      <MetricStrip items={[
        ...
        { label: 'Annual base (active)', value: <Money value={packageSummary.annualBase} /> },
      ]} />

      <div className="mb-1 flex flex-wrap items-center gap-2 border-b border-slate-200 pb-3 dark:border-slate-800">
        <div className="inline-flex rounded-md border border-slate-200 dark:border-slate-700" role="tablist" aria-label="Compensation workspace tabs">
          <button
            type="button"
            onClick={() => setTab('packages')}
            className={cn('h-9 px-4 text-sm font-semibold', tab === 'packages' ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-slate-300')}
          >
            Packages
          </button>
          <button
            ...
            className={cn('h-9 px-4 text-sm font-semibold', tab === 'changes' ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-slate-300')}
          >
            Changes
          </button>
        </div>
        <div className="relative ml-auto min-w-0 flex-1 md:max-w-sm">
          ...
          <Input value={query} onChange={event => setQuery(event.target.value)} className="pl-9" placeholder="Search employee, id, or status" />
        </div>
      </div>
      ...
      {showCreateForm && (
        <section className="border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-[#0a1422]">
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Create a draft first, then submit for approval.</p>
          <form onSubmit={handleCreateChange} className="mt-3 grid gap-3 md:grid-cols-2">
            <label className="grid gap-1.5 text-sm font-semibold">
              Employee
              <select
                value={form.employeeId}
                onChange={event => setForm(current => ({ ...current, employeeId: event.target.value }))}
                className={cn('h-10 rounded-md border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring')}
                required
              >
                <option value="">Select employee</option>
                {employees.map(employee => (
                  <option key={asString(employee.id)} value={asString(employee.id)}>
            ...
            <label className="grid gap-1.5 text-sm font-semibold">
              ...
              <select
                ...
                onChange={event => setForm(current => ({ ...current, changeType: event.target.value as ChangeType }))}
                className={cn('h-10 rounded-md border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring')}
              >
            ...
            <label className="grid gap-1.5 text-sm font-semibold">
              ...
              <Input
                ...
                onChange={event => setForm(current => ({ ...current, proposedAmount: event.target.value }))}
              />
            </label>
            <label className="grid gap-1.5 text-sm font-semibold">
              Effective date
            ...
            <div className="md:col-span-2 flex justify-end gap-2 pt-1">
              <Button variant="outline" className="h-10" type="button" onClick={() => setShowCreateForm(false)}>Cancel</Button>
              <Button className="h-10 bg-blue-600 text-white hover:bg-blue-500" disabled={changeCreateBusy} type="submit">
      ...
      {tab === 'packages' ? (
        <section className="border border-slate-200 bg-white dark:border-slate-800 dark:bg-[#0a1422]">
          <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Packages</p>
              <p className="mt-1 text-sm text-slate-500">Current and historical compensation package rows from payroll.</p>
            </div>
            {tab === 'packages' && (
              <label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                <input
                  ...
                  onChange={event => setShowInactivePackages(event.target.checked)}
          ...
          {filteredPackages.length ? <div className="overflow-x-auto">
            <table className="w-full min-w-[880px] text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {filteredPackages.map(row => (
                  <tr key={asString(row.id)} className="hover:bg-slate-50 dark:hover:bg-slate-900">
                    <td className="px-3 py-2">{asString(row.employee_name)}</td>
                    <td className="px-3 py-2 text-slate-500">{asString(row.employee_number)}</td>
                    <td className="px-3 py-2">{asString(row.job_title)}</td>
                    <td className="px-3 py-2 text-right tabular-nums"><Money value={row.base_salary} currency={asString(row.currency, 'THB')} /></td>
                    <td className="px-3 py-2 capitalize">{asString(row.pay_frequency || 'monthly')}</td>
                    <td className="px-3 py-2 text-slate-600 dark:text-slate-300">
                      {formatDate(row.effective_from)} - {row.effective_to ? formatDate(row.effective_to) : 'Ongoing'}
                    </td>
                    <td className="px-3 py-2"><PayrollStatus value={row.status} /></td>
                    <td className="px-3 py-2">
                      {canManage && <Button size="sm" variant="outline" className="h-8 border-slate-300" onClick={() => openCreateForm(row)}>New change</Button>}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <section className="border border-slate-200 bg-white dark:border-slate-800 dark:bg-[#0a1422]">
          <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Compensation changes</p>
            <p className="mt-1 text-sm text-slate-500">Review requested salary changes and move them through approval.</p>
          </div>
          {changes.length ? <div className="overflow-x-auto">
            <table className="w-full min-w-[980px] text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {changes.map(row => {
                  ...
                  const canReview = status === 'pending_approval' && canApprove;
                  return (
                    <tr key={asString(row.id)} className="hover:bg-slate-50 dark:hover:bg-slate-900">
                      <td className="px-3 py-3">
                        <p className="font-semibold">{asString(row.employee_name)}</p>
                        <p className="text-xs text-slate-500">Reason: {asString(row.reason).slice(0, 64) || '—'}</p>
                      </td>
                      <td className="px-3 py-3 text-slate-500">{asString(row.employee_number)}</td>
                      <td className="px-3 py-3 capitalize">{asString(row.change_type).replaceAll('_', ' ')}</td>
                      <td className="px-3 py-3 text-right tabular-nums"><Money value={current} currency={asString(row.currency, 'THB')} /></td>
                      <td className="px-3 py-3 text-right tabular-nums"><Money value={proposed} currency={asString(row.currency, 'THB')} /></td>
                      <td className="px-3 py-3 text-right tabular-nums">{toPercent(current, proposed)}</td>
                      <td className="px-3 py-3 text-right tabular-nums">
                        <span className={cn('text-[11px] font-semibold', current > proposed ? 'text-rose-700 dark:text-rose-300' : 'text-emerald-700 dark:text-emerald-300')}>
                          <Money value={asNumber(row.budget_impact)} currency={asString(row.currency, 'THB')} />
                        </span>
                      </td>
                      <td className="px-3 py-3">{formatDate(row.effective_date)}</td>
                      <td className="px-3 py-3"><PayrollStatus value={status} /></td>
                      <td className="px-3 py-3 max-w-[190px] truncate">{asString(row.reason)}</td>
                      <td className="px-3 py-3">
                        <div className="flex flex-wrap justify-end gap-2">
                          {canSubmit && (
                            <Button
                              ...
                              variant="outline"
                              className="h-8"
                              disabled={busy === `compensation-change-submit_change-${row.id}`}
                              onClick={() => { void handleReviewAction(row, 'submit_change').catch(() => null); }}
                            >
                              {busy === `compensation-change-submit_change-${row.id}` ? 'Submitting...' : actionLabel.submit_change}
                          ...
                          {canReview && (
                            <>
                              <Button
                                ...
                                disabled={busy === `compensation-change-approve_change-${row.id}`}
                                onClick={() => { void handleReviewAction(row, 'approve_change').catch(() => null); }}
                              >
                                {busy === `compensation-change-approve_change-${row.id}` ? 'Approving...' : actionLabel.approve_change}
                              </Button>
                              <Button
                                size="sm"
                                variant="outline"
                                className="h-8 border-rose-300 text-rose-700"
                                disabled={busy === `compensation-change-reject_change-${row.id}`}
                                onClick={() => { void handleReviewAction(row, 'reject_change').catch(() => null); }}
                              >
                                {busy === `compensation-change-reject_change-${row.id}` ? 'Rejecting...' : actionLabel.reject_change}
                              </Button>
                            </>
                          )}
                        </div>
      ...
      <div className="rounded border border-slate-200 bg-emerald-50 p-4 text-xs leading-5 text-slate-700 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-200">
        <div className="mb-2 flex items-center gap-2 font-semibold"><CheckCircle2 className="h-4 w-4" />Workspace workflow</div>
        <ol className="space-y-1">
          <li>1) Create a change draft from an active package or by selecting an employee.</li>
