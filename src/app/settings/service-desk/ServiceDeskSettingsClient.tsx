...
export function ServiceDeskSettingsClient({ canEdit }: { canEdit: boolean }) {
  ...
  return <main className="min-h-full bg-background p-4 text-foreground sm:p-5">
    <div className="w-full space-y-4">
      <header className="flex flex-col gap-4 border-b border-border pb-4  sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary ">HR setup · Employee support</p><h1 className="mt-1 text-xl font-semibold tracking-tight">Service desk</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground ">Configure employee topics, grounded AI answers, approved knowledge, and one or more HR owners.</p></div>
      ...
      {(error || message) && <div role="status" className={cn('rounded-lg border px-3 py-2 text-sm', error ? 'border-red-200 bg-red-50 text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300' : 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300')}>{error || <span className="inline-flex items-center gap-2"><Check className="h-4 w-4" />{message}</span>}</div>}
      <section className="grid overflow-hidden rounded-lg border border-border bg-card   sm:grid-cols-3"><Summary label="Categories" value={categories.length} helper="Employee-facing topics" /><Summary label="AI enabled" value={categories.filter(category => category.aiEnabled).length} helper="Grounded category assistants" /><Summary label="HR owners" value={assignedUsers} helper="People available for human support" /></section>

      <section className="grid min-h-[650px] overflow-hidden rounded-lg border border-border bg-card   lg:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="border-b border-border bg-muted/20   lg:border-b-0 lg:border-r">
          ...
          <div className="p-3">{loading ? <div className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading categories…</div> : <div className="space-y-1">{[...categories].sort((a,b) => a.sortOrder-b.sortOrder).map(item => <button key={item.key} type="button" onClick={() => setSelectedKey(item.key)} className={cn('flex w-full items-center gap-2 rounded-md px-2.5 py-2.5 text-left transition-colors', selectedKey === item.key ? 'bg-primary/10 text-primary  ' : 'hover:bg-muted dark:hover:bg-zinc-900')}><GripVertical className="h-4 w-4 shrink-0 opacity-40" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.label}</span><span className="mt-0.5 block truncate text-[11px] text-muted-foreground">{item.aiEnabled ? `${item.knowledgeDocuments.length} AI document${item.knowledgeDocuments.length === 1 ? '' : 's'}` : `${item.assigneeIds.length} HR owner${item.assigneeIds.length === 1 ? '' : 's'}`}</span></span><span className={cn('h-2 w-2 shrink-0 rounded-full', item.isActive ? 'bg-emerald-500' : 'bg-muted-foreground/40 dark:bg-zinc-600')} /></button>)}</div>}</div>
        </aside>

        <div className="p-5 sm:p-6">{selected ? <div className="max-w-3xl">
          <div className="border-b border-border pb-4 "><p className="text-xs font-semibold uppercase tracking-[0.12em] text-primary ">Category configuration</p><h2 className="mt-1 text-lg font-semibold">{selected.label}</h2><p className="mt-1 text-xs text-muted-foreground">Configure routing, AI guidance, approved knowledge, and human ownership for this topic.</p></div>
          <fieldset disabled={!canEdit || saving || uploading} className="mt-5 grid gap-5">
            ...
            <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_180px]"><label className="flex items-center justify-between gap-4 rounded-lg border border-border px-3 py-3 "><span><span className="block text-sm font-medium">Available to employees</span><span className="mt-0.5 block text-xs text-muted-foreground">Prevent new conversations without removing history.</span></span><Switch checked={selected.isActive} onCheckedChange={isActive => updateSelected({ isActive })} /></label><Field label="Display order"><Input type="number" min="0" step="10" value={selected.sortOrder} onChange={event => updateSelected({ sortOrder: Number(event.target.value) || 0 })} /></Field></div>

            <Panel title="AI assistant" description="Answer from this category's approved knowledge before offering human support." icon={<Bot className="h-4 w-4 text-primary" />} action={<Switch checked={selected.aiEnabled} onCheckedChange={aiEnabled => updateSelected({ aiEnabled })} aria-label="Enable AI assistant" />}>
...
function Panel({ title, description, icon, action, children }: { title: string; description: string; icon: ReactNode; action?: ReactNode; children: ReactNode }) { return <section className="overflow-hidden rounded-lg border border-border "><div className="flex flex-col gap-3 border-b border-border bg-muted/20 px-4 py-3   sm:flex-row sm:items-center sm:justify-between"><div><h3 className="flex items-center gap-2 text-sm font-semibold">{icon}{title}</h3><p className="mt-1 text-xs text-muted-foreground">{description}</p></div>{action}</div><div className="p-4">{children}</div></section>; }
