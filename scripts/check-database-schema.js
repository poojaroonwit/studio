...
async function checkDatabaseSchema() {
    ...
    try {
        ...
        // List of expected tables (core tables that should exist)
        ...
        for (const tableName of expectedTables) {
            try {
                ...
                if (tableExists[0].exists) {
                    ...
                    logWarning(`Table '${tableName}' is missing`);
        ...
        if (allTablesExist) {
            logSuccess('All core tables exist in the database');
        } else {
            logWarning('Some tables are missing - you may need to run migrations');
