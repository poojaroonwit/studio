...
export default function DocumentTemplatesPage() {
  ...
  return (
    <div className="flex h-full min-h-[calc(100dvh-64px)] flex-col overflow-hidden bg-background text-foreground">
      <header className="flex min-h-16 shrink-0 items-center justify-between gap-4 border-b border-border bg-card px-4  dark:bg-card sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-blue-50 text-primary dark:bg-blue-950 "><FileOutput className="h-5 w-5" /></span>
          <div className="min-w-0"><h1 className="truncate text-base font-semibold">Document templates</h1><p className="truncate text-xs text-muted-foreground dark:text-muted-foreground">Power employee self-service with ready-to-generate documents</p></div>
        </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[260px_minmax(440px,1fr)_300px]">
        <aside className="hidden min-h-0 flex-col border-r border-border bg-card  dark:bg-card lg:flex">
          <div className="space-y-3 border-b border-border p-4 ">
            <div className="flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Templates · {templates.length}</span><Button size="icon" variant="ghost" className="h-8 w-8" onClick={addTemplate} aria-label="New template"><Plus className="h-4 w-4" /></Button></div>
            <SearchBox value={query} onChange={setQuery} placeholder="Find a template" />
          </div>
          <div className="flex-1 overflow-y-auto p-2">
            {filteredTemplates.map(template => <button key={template.id} type="button" onClick={() => setSelectedId(template.id)} className={cn('mb-1 w-full rounded-md px-3 py-3 text-left transition-colors', selectedId === template.id ? 'bg-primary/10 text-primary  ' : 'hover:bg-muted/40 dark:hover:bg-muted')}><div className="flex items-start gap-2.5"><FileText className={cn('mt-0.5 h-4 w-4 shrink-0', selectedId === template.id ? 'text-primary ' : 'text-muted-foreground/70')} /><div className="min-w-0"><p className="truncate text-sm font-medium">{template.name}</p><div className="mt-1 flex items-center gap-2"><span className="text-[11px] text-muted-foreground dark:text-muted-foreground">{template.category}</span><StatusDot status={template.status} /></div></div></div></button>)}
          </div>
          <div className="border-t border-border p-3 "><Button variant="outline" className="w-full" size="sm" onClick={addTemplate}><FilePlus2 className="mr-2 h-4 w-4" />New template</Button></div>
        </aside>
          ...
          {selected ? <div className="mx-auto max-w-[820px] space-y-5">
            <div className="flex flex-col gap-4 border-b border-border pb-5  sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <Input aria-label="Template description" value={selected.description} onChange={event => updateSelected({ description: event.target.value })} className="h-auto border-0 bg-transparent px-0 text-sm text-muted-foreground shadow-none focus-visible:ring-0 dark:text-muted-foreground" />
              </div>
            ...
            <section className="space-y-2">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground dark:text-muted-foreground">Add images from the toolbar. Attributes are replaced with live data when an employee generates the document.</p>
                </div>
                <div className="inline-flex w-fit rounded-md border border-border bg-muted/40 p-0.5 dark:border-input dark:bg-muted" role="group" aria-label="Editor mode">
              ...
              {editorMode === 'visual' ? (
                <TiptapEditor value={selected.content} onChange={content => updateSelected({ content })} insertContentRequest={insertRequest} enableImages placeholder="Write the document template…" className="min-h-[520px] bg-card shadow-sm dark:bg-card [&_.ProseMirror]:min-h-[440px] [&_.ProseMirror]:px-8 [&_.ProseMirror]:py-7 [&_.ProseMirror_img]:my-4 [&_.ProseMirror_img]:max-w-full [&_.ProseMirror_img]:rounded-md" />
              ) : (
        ...
        <aside className="hidden min-h-0 flex-col border-l border-border bg-card  dark:bg-card lg:flex">
          <div className="border-b border-border p-4 "><div className="flex items-center gap-2"><Braces className="h-4 w-4 text-primary" /><h2 className="text-sm font-semibold">Attribute guide</h2></div><p className="mt-1.5 text-xs leading-5 text-muted-foreground dark:text-muted-foreground">Click an attribute to insert it at the cursor. Preview values show what employees will see.</p><div className="mt-3"><SearchBox value={attributeQuery} onChange={setAttributeQuery} placeholder="Search attributes" /></div></div>
          <div className="flex-1 overflow-y-auto p-3">{groups.map(group => { const attributes = filteredAttributes.filter(attribute => attribute.group === group); if (!attributes.length) return null; return <section key={group} className="mb-5"><h3 className="mb-1.5 px-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70">{group}</h3><div className="space-y-1">{attributes.map(attribute => <button key={attribute.key} type="button" onClick={() => insertAttribute(attribute)} className="group w-full rounded-md px-2.5 py-2 text-left hover:bg-blue-50 "><div className="flex items-center justify-between gap-2"><span className="text-xs font-medium">{attribute.label}</span><Plus className="h-3.5 w-3.5 shrink-0 text-primary opacity-0 transition-opacity group-hover:opacity-100" /></div><code className="mt-1 block truncate text-[11px] text-primary ">{`{{${attribute.key}}}`}</code><span className="mt-1 block truncate text-[11px] text-muted-foreground/70">Example: {attribute.example}</span></button>)}</div></section>; })}</div>
          <div className="border-t border-border bg-primary/10 p-4  dark:bg-blue-950/30"><div className="flex gap-2"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><p className="text-xs leading-5 text-foreground "><strong>Self-service ready.</strong> Active templates appear in My Documents. Employees generate standard letters instantly and only contact HR for exceptional requests.</p></div></div>
        </aside>
      </div>
    </div>
  );
...
function SearchBox({ value, onChange, placeholder }: { value: string; onChange: (value: string) => void; placeholder: string }) { return <div className="relative"><Search className="pointer-events-none absolute left-2.5 top-2.5 h-3.5 w-3.5 text-muted-foreground/70" /><Input value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} className="h-9 pl-8 text-xs" /></div>; }
...
function LoadingState() { return <div className="grid min-h-[calc(100dvh-64px)] place-items-center bg-[#f5f6f8] dark:bg-background"><div className="text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-primary" /><p className="mt-3 text-sm text-muted-foreground">Preparing the template studio…</p></div></div>; }
function EmptyTemplates({ onCreate, onLoad, appKitLoad }: { onCreate: () => void; onLoad: () => void; appKitLoad: { environment: 'development' | 'production'; percent: number; message: string; } | null }) { return <div className="grid min-h-[520px] place-items-center"><div className="max-w-sm text-center"><FileOutput className="mx-auto h-8 w-8 text-muted-foreground/70" /><h2 className="mt-4 font-semibold">No document templates</h2><p className="mt-2 text-sm text-muted-foreground">Load the shared catalog from AppKit or create a template here.</p><div className="mt-5 flex justify-center gap-2"><Button variant="outline" onClick={onLoad} disabled={!!appKitLoad}>{appKitLoad ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CloudDownload className="mr-2 h-4 w-4" />}{appKitLoad ? `${appKitLoad.percent}% · ${appKitLoad.message}` : 'Load from AppKit'}</Button><Button onClick={onCreate}>Create template</Button></div></div></div>; }
