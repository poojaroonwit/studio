...
export async function GET(
  ...
  try {
    ...
    const evaluation = await prisma.applicantEvaluation.findUnique({
...
export async function PUT(
  ...
  try {
    ...
    const existingEvaluation = await prisma.applicantEvaluation.findUnique({
      where: { id: evaluationId }
    });
    ...
    if (uniqueExpertiseScores && uniqueExpertiseScores.length > 0) {
      ...
      const applicantId = existingEvaluation.applicantId;
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.applicantEvaluation.delete({
      where: { id: evaluationId }
    });
