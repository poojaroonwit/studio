...
function buildEvaluateUrl(applicantId: string, token: string): string {
...
async function getActiveLink(applicantId: string) {
  const now = new Date()
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await getActiveLink(applicantId)
    if (existing && !force) {
      return NextResponse.json({
        id: existing.id,
        token: existing.token,
        url: buildEvaluateUrl(applicantId, existing.token),
    ...
    if (existing && force) {
      ...
      await prisma.applicantEvaluationLink.update({
    ...
    return NextResponse.json({
      id: created.id,
      token: created.token,
      url: buildEvaluateUrl(applicantId, created.token),
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({
      id: updated.id,
      token: updated.token,
      url: buildEvaluateUrl(applicantId, updated.token),
