...
import {
  ...
  DEFAULT_EMPLOYEE_PORTAL_DOCUMENT,
...
const COMPANY_PORTAL_SETTING_KEY = 'companyPortalBuilderState';

function isLegacyEmployeePortalDefault(state: CompanyPortalState) {
  return state.revision === 0
    && state.document.title === DEFAULT_EMPLOYEE_PORTAL_DOCUMENT.title
    && state.document.blocks[0]?.title === DEFAULT_EMPLOYEE_PORTAL_DOCUMENT.blocks[0]?.title;
}

async function lockPortalState(client: { query: (query: string, values?: unknown[]) => Promise<unknown> }) {
  await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [COMPANY_PORTAL_SETTING_KEY]);
}
...
async function readPortalState(
  ...
  const result = await client.query(
    ...
    [COMPANY_PORTAL_SETTING_KEY],
  );
  const rawValue = result.rows[0]?.value;

  if (!rawValue) return createDefaultJobPortalState();

  try {
    const state = parseCompanyPortalState(JSON.parse(rawValue), createDefaultJobPortalState());
    return isLegacyEmployeePortalDefault(state) ? createDefaultJobPortalState() : state;
  } catch {
    return createDefaultJobPortalState();
  }
}
...
async function writePortalState(
  ...
  await client.query(
    ...
    [COMPANY_PORTAL_SETTING_KEY, JSON.stringify(state)],
  );
}
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const version = {
      ...
      note: validation.data.note || 'Portal content updated',
    ...
    await logAudit(
      'AUDIT',
      `Company portal version ${revision} saved.`,
      'API:CompanyPortal:Save',
    ...
    await client.query('ROLLBACK');
    console.error('Failed to save company portal:', error);
    return NextResponse.json({ error: 'Failed to save company portal' }, { status: 500 });
  } finally {
    client.release();
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    await logAudit(
      'AUDIT',
      `Company portal version ${sourceVersion.revision} restored as version ${revision}.`,
      'API:CompanyPortal:Restore',
      session.user.id || null,
    ...
    await client.query('ROLLBACK');
    console.error('Failed to restore company portal:', error);
    return NextResponse.json({ error: 'Failed to restore company portal' }, { status: 500 });
  } finally {
    client.release();
  }
}
