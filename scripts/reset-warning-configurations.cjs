...
async function resetWarningConfigurations() {
  try {
    console.log('🔄 Starting warning configurations reset...');

    // Step 1: Delete all existing warning configurations
    console.log('🗑️  Deleting all existing warning configurations...');
    const deleteResult = await prisma.warningConfiguration.deleteMany({});
    console.log(`✅ Deleted ${deleteResult.count} existing warning configurations`);

    // Step 2: Get all users to assign configurations to
    ...
    // Step 3: Define the new warning configurations
    ...
    // Step 4: Create warning configurations for each user
    console.log('📝 Creating new warning configurations...');
    let totalCreated = 0;

    for (const user of users) {
      console.log(`👤 Creating configurations for user: ${user.name} (${user.email})`);
    ...
    console.log(`\n🎉 Warning configurations reset completed!`);
    console.log(`📊 Summary:`);
    console.log(`   - Deleted: ${deleteResult.count} existing configurations`);
    console.log(`   - Created: ${totalCreated} new configurations`);
    console.log(`   - Users: ${users.length}`);
    console.log(`   - Configurations per user: ${warningConfigurations.length}`);

  } catch (error) {
    console.error('❌ Error during warning configurations reset:', error);
    throw error;
  } finally {
...
module.exports = { resetWarningConfigurations };
