...
function transformApplicantForExport(applicant: any, jobMatches: any[]): any {
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!actingUserId) {
    await logAudit('WARN', 'Unauthorized attempt to export Applicant', 'API:Applicant:Export', null);
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }

  // Check if user has permission to export applicants
  if (!hasPermission(session.user, 'APPLICANTS_EXPORT')) {
    await logAudit('WARN', `Forbidden attempt to export Applicant by ${actingUserName}`, 'API:Applicant:Export', actingUserId);
  ...
  if (exportImportFeatureEnabled === 'false') {
    await logAudit('WARN', `Export attempt blocked - feature disabled by ${actingUserName}`, 'API:Applicant:Export', actingUserId);
  ...
  try {
    ...
    // Get Applicant with position and recruiter information
    ...
    const exportData = [transformApplicantForExport(applicant, jobMatchesResult.rows)];
    ...
    return new NextResponse(excelBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="applicant_${applicant.name}_${new Date().toISOString().split('T')[0]}.xlsx"`,
      },
    });
  } catch (error) {
