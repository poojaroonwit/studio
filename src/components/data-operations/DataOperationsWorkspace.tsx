...
function ReviewStep({ mode, model, file, format, filters, result }: { mode: 'import' | 'export'; model: DataOperationModel; file: File | null; format: ExportFormat; filters: ApplicantExportFilters; result: ImportResult | null }) {
  ...
  return <StepContent eyebrow="Step 3" title={`Review your ${mode}`} description="Nothing runs until you confirm below."><div className="divide-y rounded-lg border">
    ...
    {mode === 'import' ? <><ReviewRow label="File" value={file?.name ?? 'No file selected'} /><ReviewRow label="File size" value={file ? formatBytes(file.size) : '—'} /><ReviewRow label="Behavior" value={model.importBehavior} /></> : <><ReviewRow label="Format" value={format === 'excel' ? 'Excel workbook (.xlsx)' : format === 'jsonl' ? 'HRI transfer package (.jsonl)' : 'CSV file (.csv)'} /><ReviewRow label="Records" value={model.id === 'applicants' && activeFilters.filter(([key]) => key !== 'transferDomains').length ? `${activeFilters.filter(([key]) => key !== 'transferDomains').length} filters applied` : model.id === 'system-transfer' ? `${filters.transferDomains.split(',').filter(Boolean).length} business domains` : 'All accessible records'} /></>}
