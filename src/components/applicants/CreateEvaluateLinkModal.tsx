...
export function CreateEvaluateLinkModal({
  ...
  const validatePosition = useCallback(async () => {
    const positionId = applicant?.positionId || applicant?.position?.id;
    if (!positionId) {
  ...
  }, [applicant?.positionId, applicant?.position?.id]);
  ...
  }, [isOpen, applicant?.id, validatePosition, loadAvailableUsers, loadEmailTemplate]);
  ...
  const sendInvitationEmails = async (evaluationUrl: string) => {
    try {
      ...
      const response = await fetch(`/api/applicants/${applicant.id}/send-interview-invitation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const downloadQR = () => {
    ...
    if (canvas) {
      ...
      downloadLink.download = `evaluation-qr-${applicant.name.replace(/\s+/g, '_')}.png`;
      ...
      <div className="flex flex-col items-center py-6 space-y-6">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            For <span className={nameInfo.fontClass} lang={nameInfo.lang}>{applicant.name}</span>
          </p>
        </div>
