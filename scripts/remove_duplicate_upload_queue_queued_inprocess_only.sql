...
-- Processes ONLY queued and inprocess status entries (excludes completed/failed)
-- Keeps the earliest uploaded entry for each (file_name, position_id) combination
-- 
-- Usage: psql "$DATABASE_URL" -f scripts/remove_duplicate_upload_queue_queued_inprocess_only.sql
...
-- Step 2: Show current duplicate count for queued/inprocess status only
SELECT 
  'Current duplicate queued/inprocess upload queue entries:' as status,
...
SELECT 
  'Will delete ' || COUNT(*) || ' duplicate queued/inprocess upload queue entries' as preview
FROM duplicates_to_delete;

-- Step 4: Delete duplicates (queued/inprocess status only)
WITH duplicate_groups AS (
  SELECT 
    file_name, 
    position_id,
...
SELECT 
  'Remaining duplicate queued/inprocess upload queue entries:' as status,
...
SELECT 
  'SUCCESS: Duplicate queued/inprocess upload queue removal completed' as status,
  (SELECT COUNT(*) FROM upload_queue WHERE status IN ('queued', 'inprocess')) as active_entries,
  (SELECT COUNT(*) FROM upload_queue_backup WHERE status IN ('queued', 'inprocess')) as backup_active_count;
