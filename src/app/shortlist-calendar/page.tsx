...
export default function ShortlistCalendarPage() {
  const router = useRouter();
  const shortlistGroup = sidebarConfigData.find(g => g.label === 'Shortlist & Calendar');

  if (!shortlistGroup) return null;
  ...
  return (
    <div className="h-full flex flex-col p-6 overflow-y-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Shortlist & Calendar</h1>
          <p className="text-muted-foreground">
            Manage candidates in later stages and schedule evaluations
          </p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {shortlistGroup.items.map((item) => {
