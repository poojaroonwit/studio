...
type Enrollment = { id: string; benefitPlanId: string; status: string; effectiveFrom?: string | null; benefitPlan: Plan };
...
export function BenefitsPage() {
  ...
  const enrollmentFor = (planId: string) => data?.enrollments.find(item => item.benefitPlanId === planId && !['ended', 'cancelled', 'rejected'].includes(item.status));
  return <main className="min-h-full bg-muted/10 p-4 sm:p-6"><div className="mx-auto max-w-6xl">
    <header className="border-b border-border pb-6"><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">ESS · compensation & care</p><h1 className="mt-2 text-3xl font-bold tracking-tight">My benefits</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Compare available plans, understand monthly contributions, and request coverage for yourself or eligible dependents.</p></header>
    {loading ? <p className="py-16 text-center text-sm text-muted-foreground">Loading available benefits…</p> : !data ? <p className="py-16 text-center text-sm text-destructive">Benefits are currently unavailable.</p> : <>
      <section className="grid gap-px overflow-hidden border-x border-b border-border bg-border sm:grid-cols-3"><Summary icon={ShieldCheckIcon} value={data.plans.length} label="Available plans" /><Summary icon={CheckCircleIcon} value={data.enrollments.filter(item => item.status === 'active').length} label="Active benefits" /><Summary icon={HeartIcon} value={data.enrollments.filter(item => item.status === 'pending_approval').length} label="Pending review" /></section>
      ...
      {data.enrollments.length ? <section className="mt-9"><h2 className="text-lg font-semibold">Application history</h2><div className="mt-3 divide-y divide-border border-y border-border bg-background">{data.enrollments.map(item => <div key={item.id} className="flex items-center justify-between gap-4 px-4 py-4"><div><p className="font-semibold">{item.benefitPlan.name}</p><p className="mt-1 text-xs text-muted-foreground">Coverage from {item.effectiveFrom ? new Date(item.effectiveFrom).toLocaleDateString() : 'approval date'}</p></div><Badge variant={item.status === 'active' ? 'success' : 'secondary'}>{item.status.replace(/_/g, ' ')}</Badge></div>)}</div></section> : null}
    </>}
