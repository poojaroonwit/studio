...
| **When** | Every time a candidate applies, a stage changes, or a resume is uploaded. |
...
| **How** | 1. Access the **N8N UI** <br> 2. Open `FitScan [Process candidate].json` <br> 3. Verify the **Webhook Trigger** node is active <br> 4. Check **Execution Logs** for any AI scoring errors <br> 5. Restart failed workflows if needed |

## 1. Critical Workflow Paths
- **Inbound Processing**: Receives raw file data and converts it into structured candidate profiles.
...
> [!IMPORTANT]
> If N8N services are down, candidate profiles will still be created, but the **AI Fit Score** will remain at 0% until connectivity is restored.
