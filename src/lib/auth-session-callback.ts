...
} from './auth-callback-shared';
import type {
  AuthDbUser,
  MutableAuthToken,
  SessionCallbackInput,
...
export async function handleSessionCallback({ session, token }: SessionCallbackInput) {
  ...
  if (token.sessionToken) {
    try {
      ...
      if (isSessionValidationError(validationError)) {
  ...
  try {
    return await hydrateSessionFromTokenFallback(session, token);
  } catch (error) {
    console.error('[SESSION CALLBACK] Critical error in fallback:', error);
    return buildInactiveSession(asMutableSession(session));
  }
}
...
async function hydrateSessionFromContext(session: Session, token: MutableAuthToken, sessionToken: string) {
  ...
  session.user.role = session.user.role as UserProfile['role'];
...
async function hydrateSessionFromTokenFallback(session: Session, token: MutableAuthToken) {
  ...
  session.user.modulePermissions = session.user.id
  ...
  const impersonatedUserId = getTokenString(token, 'impersonatedUserId');
  if (impersonatedUserId) {
    session.user.impersonatedUserId = impersonatedUserId;
    session.user.adminId = getTokenString(token, 'adminId');
    session.user.name = `Preview: ${getTokenString(token, 'impersonatedName') || 'User'}`;
  }
  const impersonatedRole = getTokenString(token, 'impersonatedRole');
  if (impersonatedRole) {
    session.user.impersonatedRole = impersonatedRole as UserProfile['role'];
