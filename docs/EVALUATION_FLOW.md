# FitScan Evaluation & Scoring Flow

FitScan uses a standardized scoring system to assess candidate quality and institutionalize the grading process across the recruitment team.

---

## 📏 Scoring Framework

All candidate assessments are normalized to a **0-100% scale**, which simplifies to a clean **A through E letter grade**.
...
These are stored in the `CandidateExpertiseScore` and `CandidatePersonalityScore` tables.

### 3. AI Analysis
After a human evaluator submits their scores, the system can trigger an **AI Summary Generation** (`/api/ai/generate-content`). 
- Gemini analyzes the raw scores, recruiter comments, and the candidate's transition history.
...
| **B** | 61-80%  | Strong Candidate | `bg-lime-200` |
