...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  if (path.join('/') === 'attendance/corrections') return createMobileAttendanceCorrection(identity, body);
  if (path.join('/') === 'profile-change-requests') return createMobileProfileChangeRequest(identity, body);
  ...
  if (path.join('/') === 'emergency-contacts') {
    ...
    return 'error' in result ? jsonError(result.error || 'Unable to save emergency contact', result.status || 400) : NextResponse.json(result.contact, { status: 201 });
  }
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  if (path[0] === 'emergency-contacts' && contactId) {
    ...
    return 'error' in result ? jsonError(result.error || 'Unable to save emergency contact', result.status || 400) : NextResponse.json(result.contact);
  }
  return jsonError('Not found', 404);
}
