...
export const getTraitScoresByEvaluator = (traitId: string, allEvaluations: EvaluationRecord[]) => {
    ...
    allEvaluations.forEach(evaluation => {
        const traitScore = evaluation.personalityScores?.find(score => score.trait?.id === traitId);
...
export const getEvaluatorsForGroup = (group: GroupedTrait, allEvaluations: EvaluationRecord[]) => {
