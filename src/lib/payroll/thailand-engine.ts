...
export type ThaiPayrollInput = z.infer<typeof thaiPayrollInputSchema>;
...
export function progressiveTax(annualTaxableIncome: number, rules = defaultThaiPayrollRules) {
  let previousLimit = 0;
  let remaining = Math.max(0, annualTaxableIncome);
  let tax = 0;
  const trace: Array<{ from: number; to: number | null; rate: number; taxable: number; tax: number }> = [];

  for (const bracket of rules.taxBrackets) {
    const capacity = bracket.upTo === null ? remaining : Math.max(0, bracket.upTo - previousLimit);
    const taxable = Math.min(remaining, capacity);
    const bracketTax = taxable * bracket.rate;
    trace.push({ from: previousLimit, to: bracket.upTo, rate: bracket.rate, taxable: round(taxable), tax: round(bracketTax) });
    tax += bracketTax;
    remaining -= taxable;
...
export function calculateThaiPayroll(raw: ThaiPayrollInput, rules = defaultThaiPayrollRules) {
  const input = thaiPayrollInputSchema.parse(raw);
  const grossPay = input.earnings.baseSalary
  ...
  const socialSecurityBase = Math.min(
    Math.max(0, grossPay - input.earnings.bonus - input.earnings.terminationPay),
    rules.socialSecurityMonthlyWageCeiling,
  );
  const employeeSocialSecurity = round(socialSecurityBase * rules.employeeSocialSecurityRate, rules.roundingDecimals);
  const employerSocialSecurity = round(socialSecurityBase * rules.employerSocialSecurityRate, rules.roundingDecimals);
  const providentFundEmployee = round(input.earnings.baseSalary * input.deductions.providentFundEmployeeRate, rules.roundingDecimals);
  const providentFundEmployer = round(input.earnings.baseSalary * input.deductions.providentFundEmployerRate, rules.roundingDecimals);
  const currentTaxableIncome = Math.max(
    0,
    grossPay - input.deductions.unpaidLeave - input.deductions.otherPreTax - employeeSocialSecurity - providentFundEmployee,
  );
  const projectedAnnualIncome = input.yearToDate.taxableIncome
    + currentTaxableIncome
    + Math.max(0, input.period.periodsPerYear - 1) * Math.max(0, input.earnings.baseSalary - employeeSocialSecurity - providentFundEmployee);
  const annualTaxableIncome = Math.max(0, projectedAnnualIncome - input.annualDeductions);
  const annualTax = progressiveTax(annualTaxableIncome, rules);
  const pitWithholding = round(Math.max(0, (annualTax.tax - input.yearToDate.pitWithheld) / input.period.periodsPerYear), rules.roundingDecimals);
  const totalEmployeeDeductions = input.deductions.unpaidLeave
    + input.deductions.otherPreTax
    + input.deductions.otherPostTax
    + employeeSocialSecurity
    + providentFundEmployee
    + pitWithholding;
  const netPay = round(Math.max(0, grossPay - totalEmployeeDeductions), rules.roundingDecimals);

  return {
    jurisdiction: 'TH',
    legalVersion: rules.legalVersion,
    authoritative: false,
    reviewNotice: 'Draft statutory rules require approval by a qualified Thai payroll or legal reviewer before production use.',
    ...
    employerCost: round(grossPay + employerSocialSecurity + providentFundEmployer),
    netPay,
    components: {
