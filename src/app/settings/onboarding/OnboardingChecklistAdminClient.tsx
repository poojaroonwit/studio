...
export function OnboardingChecklistAdminClient({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <main className={cn('min-h-full bg-[#f7f9fc] text-[#172033]', embedded ? 'p-0' : 'px-4 py-7 sm:px-6 lg:px-8')}>
      <div className={cn('overflow-hidden border-border bg-white', embedded ? 'min-h-[720px] border-0' : 'rounded-xl border')}>
        <header className="flex flex-col gap-4 border-b border-border px-4 py-4 sm:px-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <label className="relative min-w-0">
              ...
              <ChevronDownIcon className="pointer-events-none absolute right-1 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground/70" />
            </label>
            <span className={cn('rounded-md border px-2 py-0.5 text-[11px] font-semibold', isPublished ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300' : 'border-input bg-muted text-muted-foreground dark:border-input dark:text-foreground/80')}>{isPublished ? 'Published' : 'Draft'}</span>
            <span className="shrink-0 text-xs text-muted-foreground">{templateTasks.length} steps</span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="relative min-w-[190px] flex-1 xl:w-[220px] xl:flex-none"><MagnifyingGlassIcon className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground/70" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search steps..." className="h-9 pl-9" /></label>
            <Button asChild variant="outline" size="sm" className="h-9 dark:text-blue-300"><Link href="/people/onboarding"><EyeIcon className="mr-2 h-4 w-4" />Preview journey</Link></Button>
            ...
            <Button type="button" variant="outline" size="sm" className="h-9 dark:text-blue-300" onClick={() => importInputRef.current?.click()} disabled={!selectedTemplateId || isSaving}><ArrowDownTrayIcon className="mr-2 h-4 w-4" />Import steps</Button>
            <Button type="button" size="sm" className="h-9 bg-primary px-5 hover:bg-primary/90" onClick={() => void publishTemplate()} disabled={!selectedTemplate || !templateTasks.length || isSaving}>{isPublished ? 'Republish template' : 'Publish template'}</Button>
          </div>
        </header>

        {showNewTemplate && <div className="flex flex-wrap items-center gap-2 border-b border-border bg-muted/40 px-5 py-3"><Label htmlFor="new-template" className="mr-2">New template</Label><Input id="new-template" autoFocus value={newTemplateName} onChange={event => setNewTemplateName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void createTemplate(); }} placeholder="Template name" className="h-9 max-w-sm" /><Button size="sm" disabled={!newTemplateName.trim() || isSaving} onClick={() => void createTemplate()}>Create draft</Button><Button size="icon" variant="ghost" onClick={() => setShowNewTemplate(false)} aria-label="Cancel new template"><XMarkIcon className="h-4 w-4" /></Button></div>}
        {(error || notice) && <div className={cn('border-b px-5 py-2.5 text-sm', error ? 'border-rose-200 bg-rose-50 text-rose-700 dark:bg-rose-950/30 dark:text-rose-300' : 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-300')} role={error ? 'alert' : 'status'}>{error || notice}</div>}

        {selectedTaskIds.size > 0 && <div className="flex flex-wrap items-center gap-2 border-b border-border bg-muted/40 px-5 py-3"><span className="mr-2 text-sm font-medium">{selectedTaskIds.size} selected</span><Button size="sm" variant="outline" onClick={() => void duplicateTasks([...selectedTaskIds])} disabled={isSaving}><DocumentDuplicateIcon className="mr-2 h-4 w-4" />Duplicate</Button><select value={bulkPhase} onChange={event => setBulkPhase(event.target.value as PhaseId)} className="h-9 rounded-md border border-input bg-background px-3 text-sm">{PHASES.map(phase => <option key={phase.id} value={phase.id}>Move to {phase.label}</option>)}</select><Button size="sm" variant="outline" onClick={() => void moveTasksToPhase([...selectedTaskIds], bulkPhase)} disabled={isSaving}>Move</Button><Button size="sm" variant="outline" className="text-rose-600" onClick={() => void removeTasks([...selectedTaskIds])} disabled={isSaving}><TrashIcon className="mr-2 h-4 w-4" />Delete</Button><button type="button" className="ml-auto text-xs font-semibold text-primary" onClick={() => setSelectedTaskIds(new Set())}>Clear selection</button></div>}

        <div className="overflow-x-auto">
          <div className="min-w-[980px]">
            <div className="grid grid-cols-[42px_54px_minmax(260px,1fr)_160px_190px_96px_150px_120px] items-center border-b border-border bg-muted/40 px-4 py-2.5 text-[11px] font-semibold text-muted-foreground dark:text-muted-foreground">
            ...
            {PHASES.map(phase => {
              ...
              return <section key={phase.id} aria-labelledby={`phase-${phase.id}`}>
                <div className="flex items-center border-b border-border bg-muted/40 px-4 py-2.5"><ChevronDownIcon className="mr-2 h-4 w-4 text-muted-foreground/70" /><h2 id={`phase-${phase.id}`} className="text-sm font-semibold">{phase.label}</h2><span className="ml-2 text-xs text-muted-foreground">({phaseTasks.length})</span><Button type="button" variant="ghost" size="sm" className="ml-auto h-7 text-xs text-primary dark:text-blue-300" onClick={() => void createTask(phase.id)} disabled={!selectedTemplateId || isSaving}><PlusIcon className="mr-1 h-3.5 w-3.5" />Add step</Button></div>
                {phaseTasks.map((task, phaseIndex) => {
                  ...
                  return <React.Fragment key={task.id}>
                    <article draggable onDragStart={() => setDraggingTaskId(task.id)} onDragEnd={() => setDraggingTaskId(null)} onDragOver={event => event.preventDefault()} onDrop={() => { if (draggingTaskId) void reorderTask(draggingTaskId, task.id); setDraggingTaskId(null); }} onClick={() => beginEdit(task)} className={cn('grid cursor-pointer grid-cols-[42px_54px_minmax(260px,1fr)_160px_190px_96px_150px_120px] items-center border-b border-border px-4 py-2.5 text-xs transition-colors hover:bg-muted/40', editingTaskId === task.id && 'bg-blue-50/60 ring-1 ring-inset ring-ring dark:bg-blue-950/20', draggingTaskId === task.id && 'opacity-50')}>
                      ...
                      <span className="flex items-center gap-2 text-muted-foreground/70"><Bars3Icon className="h-4 w-4 cursor-grab" />{phaseIndex + 1}</span>
                      <span className="truncate font-medium">{String(task.title || 'Untitled checklist step')}</span>
                      <span className="flex items-center gap-1.5 capitalize text-muted-foreground dark:text-foreground/80">{String(value(task, 'ownerRole', 'owner_role') || 'employee')}</span>
                      <span className="text-muted-foreground dark:text-foreground/80">{phaseLabel(task)}</span>
                      <span><span className={cn('rounded border px-2 py-0.5 text-[11px] font-semibold', required ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300' : 'border-input text-muted-foreground dark:border-input')}>{required ? 'Yes' : 'No'}</span></span>
                      <span className="capitalize text-muted-foreground dark:text-foreground/80">{visibility.replace(/_/g, ' ')}</span>
            ...
            {!isLoading && !templateTasks.length && <div className="px-6 py-16 text-center"><h2 className="text-lg font-semibold">No checklist steps yet</h2><p className="mt-2 text-sm text-muted-foreground">Add the first step to Before start, First week, or First 30 days.</p><Button className="mt-5" onClick={() => void createTask('before_start')} disabled={!selectedTemplateId || isSaving}><PlusIcon className="mr-2 h-4 w-4" />Add first step</Button></div>}
            {isLoading && <div className="space-y-1 p-4">{Array.from({ length: 7 }).map((_, index) => <div key={index} className="h-10 animate-pulse rounded bg-muted" />)}</div>}
          </div>
        </div>
      </div>
    </main>
...
function TaskEditor({ draft, setDraft, isSaving, onCancel, onSave }: {
  ...
  return <div className="grid gap-x-8 gap-y-4 border-b border-border bg-muted/30 px-11 py-5 lg:grid-cols-2">
