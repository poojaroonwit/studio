...
export async function POST(
  ...
  // Check permissions - user needs to be able to view applicants at minimum
  ...
  try {
    ...
    if (!positionId) {
      await client.query('ROLLBACK');
      return NextResponse.json(
        { message: 'Applicant is not associated with a position' },
        { status: 400 }
      );
    }
    ...
    for (const interviewer of interviewers) {
      try {
        ...
        const calendarContent = generateCalendarInvite({
          title: `Interview: ${applicant.name} - ${position.title}`,
        ...
        if (emailResult.success) {
          ...
          try {
            const calendarResult = await createCalendarEvent({
              ...
              subject: `Interview: ${applicant.name} - ${position.title}`,
              body: `<p>${notes || `Interview with ${applicant.name} for position ${position.title}.`}</p>${evaluationLink ? `<p><strong>Evaluation Link:</strong> <a href="${evaluationLink}">${evaluationLink}</a></p>` : ''}`,
          ...
          await logAudit(
            'AUDIT',
            `Interview invitation sent to ${interviewer.userName} (${interviewer.userEmail}) for Applicant ${applicant.name} by ${session.user.name || session.user.email}`,
    ...
    if (locationEmail) {
      try {
        const roomBookingResult = await createCalendarEvent({
          ...
          subject: `Interview: ${applicant.name} - ${position.title}`,
          body: `<p>Interview with ${applicant.name} for position ${position.title}.</p><p><strong>Interviewer(s):</strong> ${interviewers.map(i => i.userName).join(', ')}</p>`,
          startDateTime: interviewDateTime,
          endDateTime: endDateTime,
          location: location || '',
          organizerName: organizer.name,
