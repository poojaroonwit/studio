...
import { AccountRolesTab, UsersPageHeader, UsersPageTabs, type UsersSettingsTabId } from './UsersPageParts';
...
export function ManageUsersPageContent({ accountsOnly = false }: { accountsOnly?: boolean }) {
  ...
  return <div className="flex h-full flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-auto px-6 pb-6"><div className="h-full min-h-0">{activeTab === 'users' && <OutbornAccountDirectoryPanel />}{!accountsOnly && activeTab === 'groups' && <AccountRolesTab />}</div></div>
  </div>;
}
