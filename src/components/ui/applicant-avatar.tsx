  ...
  useEffect(() => {
    ...
    const loadAvatar = async () => {
      try {
        ...
        console.warn('[Applicant_AVATAR] Failed to load avatar:', error);
        if (isMountedRef.current && !isCancelled) {
          setImageUrl(null);
          setIsLoading(false);
          setImageLoaded(false);
...
export function ApplicantAvatarCompact({ user, size = 'sm', className, forceRefresh }: ApplicantAvatarProps) {
  return (
    <ApplicantAvatar
      user={user}
      size={size}
  ...
  useEffect(() => {
    ...
    const loadAvatar = async () => {
      try {
        ...
        console.warn('[Applicant_AVATAR_LARGE] Failed to load avatar:', error);
        if (isMountedRef.current && !isCancelled) {
          setImageUrl(null);
          setIsLoading(false);
        }
