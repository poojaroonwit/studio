...
const UploadAttachmentsModal = ({ isOpen, onOpenChange, applicant, onUploadSuccess }: UploadAttachmentsModalProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const uploadPromises = filesWithTags.map(async ({ file, tag }) => {
        ...
        const response = await fetch(`/api/applicants/${applicant.id}/resumes`, {
          method: 'POST',
          body: formData,
        });
