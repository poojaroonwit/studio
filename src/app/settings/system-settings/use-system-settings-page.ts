...
export function useSystemSettingsPage() {
  ...
  const fetchSystemSettings = useCallback(async () => {
    ...
    try {
      ...
      applySystemSettingsViewState(parseSystemSettingsViewState(settings));
    } catch (error) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const settingsToSave = buildSystemSettingsSavePayload(formState);

    try {
      await saveSystemSettings(settingsToSave);
      toast.success('Settings Saved');
      publishAppConfigChange(settingsToSave);
      fetchSystemSettings();
    } catch (error) {
